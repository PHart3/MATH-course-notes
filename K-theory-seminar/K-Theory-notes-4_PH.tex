...
Thanks to Whitehead, we know that the commutator subgroup $\left[\GL(R), \GL(R)\right]$ is equal to $E(R) = \bigcup_n E_n(R)$, the group of elementary matrices $E_{i, j}(r)$ where $r \in R$ and $i\ne j$. (The matrix $E_{i, j}(r)$ is the identity with the $\left(i,j\right)$-entry replaced by $r$.) Thus, $K_1(R)$ can be viewed as the ``stabilized'' group of automorphisms of the trivial projective module modulo trivial automorphisms.
