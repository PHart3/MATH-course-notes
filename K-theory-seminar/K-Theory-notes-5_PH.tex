...
\medskip
 ...
 Moreover,
let $F : \left(\c, w{\c}\right) \to \left(\d, w\d\right)$ be an exact functor. Set $K(F) = \Omega\lvert{wS_{\bullet}F}\rvert : K(\c, w) \to K(\d, w)$. This defines the \textit{algebraic $K$-theory functor} $K : \mathbf{Wald} \to \mathbf{Top_{\ast}}.$
...
\begin{exmp}
Let $X$ be a CW complex and $\mathcal{R}(X)$ denote the category of CW complexes $Y$  obtained from $X$ by attaching at least one cell such that $X$ is a retract of $Y$. Equip this with cofibrations in the form of cellular inclusions fixing $X$ and w.e. in the form of homotopy equivalences. This makes $\mathcal{R}(X)$ into a Waldhausen category. 
