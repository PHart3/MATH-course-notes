...
\item The \textit{category of cones under $F$} is the left fiber category  $F/X$.
\end{enumerate}
\end{definition}
...
\begin{exmp}  
If $\c$ is the empty category, then the empty functor $F: \c \to \d$ satisfies $F/\Delta_{\c}\cong \d$, so that the colimit of $F$ is precisely the  initial object of $\d$.
\end{exmp}
...
Note that the colimit of a functor $F$ is the limit of $F^{\op}$. Hence \textit{limit} and \textit{colimit} are dual properties, and our results so far for colimits can be dualized  for limits.
...
Consider any small diagram $F: \mathscr{J} \to \set$. On the one hand,
\[
...
\]
We have proven that $\set$ is both complete and cocomplete.
\end{exmp}

\medskip

Let $\j$ be a category of the form $\bullet \rightrightarrows \bullet$. Then  a diagram $D$ of shape $\j$ looks like $A \overset{f}{\underset{g}{\rightrightarrows}} B$. A cone over $D$ with apex $C$ and legs $f_1 : C \to A$ and $f_2 : C \to B$ satisfies $ff_1=f_2 = gf_1$.
...
\item Dually, the colimit of $D$ is called the \textit{coequalizer} of $f$ and $g$.
\end{enumerate}
\end{definition}
...
The coequalizer of $\left(f, g\right)$ is $\faktor{Y}{\sim}$ together with the quotient map on $B$ where $\sim$ is the smallest equivalence relation under which $f(x) \sim g(x)$ for every $x$.
\end{exmp}
...
If such an object $C$ together with $i$ and $j$ is the limit of this diagram, then we call it the \textit{pullback} of $f$ and $g$, denoted by $B \times_{C} A$.
\end{definition}


We can perform an analogous construction for $\j^{\op}$. Then the colimit of the resulting diagram is called the \textit{pushout}, denoted by $B \cup_{C} A$.
...
\begin{enumerate}
\item Given a morphism $p: 1 \to Y$, the \textit{fiber $f^{{-1}}(p)$ of $f$}  at $p$ is the pullback  of the cospan $1 \rightarrow Y \leftarrow X$.
