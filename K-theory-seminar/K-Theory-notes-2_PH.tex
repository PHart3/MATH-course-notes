...
\begin{abstract}
We  explore the concept of a universal property in category theory. The main sources for this talk are the following.
\begin{itemize}
\item $n$Lab
...
We can think of a universal property as follows.  Let $F : \d \to \c$ be a functor and $X \in \ob \c$. A \textit{universal arrow from $X$ to $F$} is an ordered pair $\left(Y, f\right)$ with $Y \in \ob \d$ and $f : X \to F(Y)$ a morphism of $\c$ with the property that for any $X' \in \ob \d$ and morphism $f' : X \to F(X')$ of $\c$, there exists a unique morphism $\hat{f} : Y \to X'$ of $\d$ such that $F\big(\hat{f}\big) \circ f = f'$.
\[ \begin{tikzcd}
...
\begin{enumerate}
\item The \textit{slice} or \textit{left fiber category}, denoted by $\left(F/Y\right)$ or $\left(F \downarrow Y\right)$, has as objects pairs $\left(X, f\right)$ where $f: F(X) \to Y$ and as morphisms from $f : F(X) \to Y$ to $f' : F(X') \to Y$ morphisms $g : X \to X'$ such that $f = f' \circ F(g).$ 
\item 
...
\begin{prop}
Let $F : \d \to \c$ be a functor and $x \in \ob C$. Then $u : x \to Fr$ is a universal arrow from $x$ to $F$ iff it is an initial object of  $\left(x \downarrow F\right)$. Dually, $u' : Fr' \to x$ is a universal arrow from $F$ to $x$  iff it is a terminal object of $\left(x \downarrow F\right)$.
\end{prop}
\begin{proof}
Suppose that $u$ is universal and $f: x \to Fy$ is another object of $\left(x \downarrow F\right)$. Then there exists a unique $\hat{f}: r \to y$ such that $F\big(\hat{f}\big) \circ u = f$. Thus $F\big(\hat{f}\big)$ is a unique morphism of the coslice.
...
\begin{exmp}
The unique initial object of $\set$ is $\emptyset$, and the terminal objects are precisely the singleton sets. Hence there is no zero object. Moreover, there is no initial or terminal object in $\iso(\set)$.
\end{exmp}


For any $X \in \ob \c$, the \textit{undercategory} ${X}/{\c}$ has as objects morphisms in $\c$ of the form $i : X \to Y$ where $X$ is fixed. Given $i: X \to Y$ and  $i' : X \to Y'$ in $\ob {X}/{\c}$, define the set of morphisms from $i$ to $i'$ as the morphisms $f: Y \to Y'$ where
\[ \begin{tikzcd}
...
\begin{definition}
For any functor $F: \j \to \c$ and $X \in \ob \c$, a \textit{cone over $F$} consists of an \textit{apex} $X\in \ob \c$ and \textit{legs} $f_j : X \to F(j)$  ($j \in \ob \j$) such that for any morphism $\alpha : j \to j'$, the triangle
\[ \begin{tikzcd}
...
A cone over $F$ is just a natural transformation $\Delta_{\j}{X} \Rightarrow F$ where $\Delta_{\j} X$ denotes the constant functor on $\j$ at $X$. (If $\j$ is small, then $\Delta_{\j}$ is a functor from $\c$ to $\mathbf{Fun}(\j, \c)$.) The notion of \textit{cocone under $F$} is dual to that of cone.

\begin{definition}[Colimit]
Let $\c$ and $\d$ be categories and $g: Y \to Z$ be a morphism in $\d$. Let $\Delta_{\c} g : \Delta_{\c} Y \Rightarrow \Delta_{\c} Z$ be the natural transformation where every component is exactly $g$. 
\begin{enumerate}
\item A \textit{colimit  $\colim_{\c}{F}$} of a functor $F: \c \to \d$ is an object $Y$ of $\d$ together with a natural transformation $i : F \Rightarrow \Delta_{\c} Y$ such that for any $Z \in \ob \d$ and any natural transformation $j: F \Rightarrow \Delta_{\c} Z$, there is a unique morphism $g: Y \to Z$ such that $j = \Delta_{\c}g \circ i$.
\item  We say that $\d$ \textit{admits/has  $\c$-shaped colimits} if each functor $G: \c \to \d$ has a colimit.
\item We say that $\d$ is \textit{cocomplete} if it admits $\c$-shaped colimits whenever $\c$ is small.
\end{enumerate}
\end{definition}

If $\c$ is small, then a colimit of $F: \c \to \d$ is just an initial object in the right fiber category $F/\Delta_{\c}$, which has as objects pairs $\left(Z, j: F \to \Delta_{\c}{Z}\right)$ and as morphisms from $\left(Y, i\right)$ to $\left(Z, j\right)$ morphisms $g : Y \to Z$ in $\d$ such that $\Delta_{\c}{g} \circ i = j$.

\smallskip

\begin{exmp} 
If $\c$ is the empty category, then the  functor $F: \c \to \d$ satisfies $F/\Delta_{\c}\cong \d$, so that the colimit of $F$ is exactly the  initial object of $\d$.
\end{exmp}
...
\begin{lemma}
Any colimit of a functor is unique up to unique isomorphism. 
\end{lemma}
\begin{proof}
...
\begin{remark}
Assume that $\d$ has $\c$-shaped colimits and that $\c$ is small. Then a (possibly global) choice function $\colim_\c : \mathbf{Fun}(\c, \d) \to \d$ given by choosing a colimit for each functor induces a functor that is left adjoint to the constant diagram functor $\Delta_{\c} : \d \to \mathbf{Fun}(\c, \d)$. Indeed,
...
The \textit{limit} of the functor $F: \c \to \d$ is  the colimit of $F^{\op} : \c^{\op} \to \d^{\op}$.
\end{definition}
...
Note that the colimit of a functor $F$ is exactly the limit of $F^{\op}$. Hence \textit{limit} and \textit{colimit} are dual properties, and all of our results for colimits can be dualized  for limits.

\begin{definition}
Let  $F : \c \to \d$ be a functor. We say that $F$ \textit{creates limits} if
\begin{itemize}
\item for any functor  $G : \j \to \c$, $G$ has a limit when $F \circ G$ has a limit;
\item $F$ preserves all limits of $G$; and
\item $F$ reflects all limits of $G$.
\end{itemize}
\end{definition}
