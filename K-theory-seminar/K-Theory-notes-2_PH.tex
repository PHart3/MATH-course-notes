...
\begin{remark}
If $X \in \ob \c$, then $\left({X}/{\c}\right)^{\op} = {\c^{op}}/{X}$. Thus, the under- and overcategories are dual in the sense that $P(X, \c)$ is true for any undercategory ${X}/{\c}$ iff $P^{\op}(X, \c)$ is true for any overcategory ${\c}/{X}$. 
\end{remark}
...
\begin{enumerate}
\item A \textit{colimit} for the functor $F: \c \to \d$ consists of an object $Y$ of $\d$ and a natural transformation $i : F \Rightarrow \Delta_{\c} Y$ such that for any $Z \in \ob \d$ and natural transformation $j: F \Rightarrow \Delta_{\c} Z$, there is a unique morphism $g: Y \to Z$ such that $j = \Delta_{\c}g \circ i$. We write $Y = \colim_{\c} F$.
\item  We say that $\d$ admits \textit{all $\c$-shaped colimits} if each functor $G: \c \to \d$ has a colimit and that $\d$ is \textit{cocomplete} if each functor $G : \c \to \d$ with $\c$ small has a colimit.
\end{enumerate}
\end{definition}
...
\begin{lemma}
Assume that $\d$ admits all $\c$-shapes colimits and that $\c$ is small. Then a (possibly global) choice function $\colim_\c : \mathbf{Fun}(\c, \d) \to \d$ given by choosing a colimit for each functor determines a functor that is left adjoint to the constant diagram functor $\Delta_{\c} : \d \to \mathbf{Fun}(\c, \d)$.
\end{lemma}
\begin{proof}
...
\begin{definition}
A \text{limit} of the functor $F: \c \to \d$ is precisely the colimit of $F^{op} : \c^{op} \to \d^{op}$.
\end{definition}
...
\begin{definition}
Let $\j$ be a discrete small category. Consider a diagram  $\left\{A_i\right\}_{i\in J}$ of shape $\j$. 
\begin{enumerate}
\item A limit for this diagram is the \textit{product} $\prod_i A_i$ equipped with projections $\pi_i : \prod_i A_i \to A_i$ such that for every $f_{i} : U \to A_i$ there is some unique $f: U \to \prod_i A_i$ with $\pi_i \circ f = f_i$. 
\item  Dually, a colimit for the diagram is the \textit{coproduct} $\sum_i A_i$ equipped with inclusions $u_i : A_i \to \sum_i A_i$ such that for any $f_i : A_i \to Y$, there is some unique $f : \sum_i A_i \to Y$ with $f_i = f \circ u_i$.
\end{enumerate}
\end{definition}
...
\begin{definition}
If such an object $C$ together with $f_1$ is the limit of the diagram, then we say it is the \textit{equalizer} of $f$ and $g$. Dually, a colimit is called the \textit{coequalizer}.
\end{definition}

\begin{exmp}
The equalizer in $\set$ of $f, g: X \to Y$ is the subset $X'\coloneqq \left\{x \in X : f(x) = g(x)\right\}$ together with the inclusion $X' \hookrightarrow X$. The coequalizer of $(f, g)$ is $\faktor{Y}{\sim}$ together with the quotient map on $B$ where $\sim$ is the smallest equivalence relation under which $f(x) \sim g(x)$ for every $x$.
\end{exmp}

\begin{remark}
The same idea applies to $\mathbf{Grp}$. The relation $\sim$ just becomes a certain minimal normal subgroup.
\end{remark}
...
\end{prop}
\begin{proof}
We have the following chain of natural bijections for each $Y \in \d$: 
\begin{align*}
 \d(F(\colim_{\e}), Y) & \cong \c(\colim_{\e} X, G(Y)) 
 \\ & \cong \lim_{\e} \c(X(-), G(Y))
 \\ &  \cong \lim_{\e} \d(F(X(-)), Y)
 \\ &  \cong \mathbf{Fun}(\e, \d)(F \circ X, \Delta Y).
 \end{align*} The second bijection follows from the fact that both sets can be identified with the components of the natural transformations from $X$ to $\Delta G(Y)$.
\end{proof}
...
\begin{definition}
Suppose $\c$ has terminal object $1$. A \textit{cofiber} of a morphism $f: X \to Y$ is a pushout of the diagram $1 \leftarrow X \rightarrow Y$. We write $Y/X$. Further, given a morphism $p: 1 \to Y$, the \textit{fiber} of $f$ at $p$ is the pullback of $1 \rightarrow Y \leftarrow X$, written as $f^{{-}1}(p)$.
\end{definition}

\smallskip

Let $F : \c \to \d$ be a functor. For each $Y \in \ob \d$, there is a full and faithful functor $F^{{-}1}(Y)  \rightarrowtail F/Y$ given by $X \mapsto \left(X, \id_Y\right)$. 

\begin{definition} $ $
We say that $\c$ is a \textit{precofibered category} over $\d$ if this functor admits a left adjoint given by $\left(Z, g: F(Z) \to Y\right) \mapsto g_{\ast}(Z)$.
\end{definition}

Moreover, there is a full and faithful functor $F^{{-}1}(Y) \rightarrowtail Y/F$ defined similarly. We say that $\c$ is a \textit{prefibered category} over $\d$ if this functor admits a right adjoint given by $\left(Z, g: Y \to F(Z)\right) \mapsto g_{\ast}(Z)$.
