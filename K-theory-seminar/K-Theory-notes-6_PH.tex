...
\end{theorem}
\begin{proof}
As $S_2{\c}$ is universal, it suffices to prove that $t_{\ast}  \simeq s_{\ast} + q_{\ast}$. Notice that the two composites $$\c \times \c \overset{\coprod}{\longrightarrow} S_2{\c} \xbigtoto[s \coprod q]{t} \c$$ are the same. \Cref{ext} implies that $K\left(\coprod\right): K(\c) \times K(\c) \to K(S_2{\c})$ is a homotopy equivalence. Since the $H$-space structure on $K(\c)$ is induced by $\coprod$, we get $t_{\ast}  \simeq s_{\ast} + q_{\ast}$. 
\end{proof}
