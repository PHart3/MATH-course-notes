...
\begin{abstract}
Even more basic category theory. The main sources for these notes are nLab, Rognes, Ch. 4, and Peter Johnstone's Part III lecture notes (Michaelmas 2015), Ch. 4.
\end{abstract}
...
\begin{exmp}
The equalizer in $\set$ of $f, g: X \to Y$ is the subset $X':= \{x \in X : f(x) = g(x)\}$ together with the inclusion $X' \hookrightarrow X$. The coequalizer of $(f, g)$ if $\faktor{Y}{\sim}$ together with the quotient map on $B$ where $\sim$ is the smallest equivalence relation under which $f(x) \sim g(x)$ for every $x$.
\end{exmp}
