...
\medskip
 ...
 Consider $S\coloneqq \faktor{C \times C}{\sim}$ where $\left(a_1, b_1\right) \sim \left(a_2, b_2\right)$ if $$a_1 + b_2 +k = b_1 + a_2 +k$$ for some $k\in C$. Note that $\sim = {\sim'}$ where $\left(a_1, b_1\right) \sim' \left(a_2, b_2\right)$ if $$\left(a_1 + k_1, b_1 + k_1\right) = \left(a_2 +k_2, b_2 + k_2\right)$$ for some $\left(k_1, k_2\right) \in C\times C$. Now define $K(C) = \left(S, +\right)$, where $+$ is inherited from $C$ and acts componentwise on equivalence classes. Our definition of ${\sim'}$ makes it clear that $\left[a_1, b_1\right]^{-1} = \left[b_1, a_1\right]$.
...
\begin{prop}\label{prop2} Suppose that $L$ is a cofinal submonoid of $C$.
\begin{enumerate}
...
\item If $\left[m\right] = \left[m'\right]$, then $m + l = m' +l$ for some $l\in L$.
\end{enumerate}
\end{prop}
...
Let $R^{\infty} = \coprod_{n\geq 1}R$. Suppose that $P \oplus Q =R^n$ as $R$-modules. Then $$P \oplus R^{\infty} \cong P \oplus  \left(Q \oplus P\right) \oplus \left(Q \oplus P\right) \oplus \cdots \cong \left(P \oplus Q\right) \oplus \left(P \oplus Q\right) \oplus \cdots \cong R^{\infty}.$$
\end{theorem}

Therefore, if we added $R^{\infty}$ to $\P(R)$, then we would have $\left[P\right] = 0$ for each finitely generated projective module $P$.
...
\item Suppose $R$ has the IBP. Then $K_0(R) \cong \Z$ iff every finitely generated projective $R$-module is stably free.
\end{enumerate}
\end{lemma}
\begin{proof} $ $
\begin{enumerate}
...
\begin{exmp}
Suppose that $R$ is commutative. There is a ring homomorphism $R\to F$ with $F$ a field. Then the induced map $K_0(R) \to K_0(F) \cong \Z$ sends $\left[R\right]$ to $1$. Also, the map $\phi : \Z \to K_0(R)$ is injective by \cref{L4}. Letting $K= \ker(K_0(R) \to \Z)$, we get a split exact sequence of abelian groups
\[
\begin{tikzcd}
...
\begin{exmp}
A ring $R$ is a \textit{flasque} if there exists an $R$-bimodule $M$ that is a finitely generated projective module on one side along with a bimodule isomorphism $R\oplus M \cong M$. In this case, since $$P \oplus \left(P \otimes_R M\right) \cong P \otimes_R \left(R \oplus M\right) \cong P \otimes_R M,$$ we see that $K_0(R) =0$.
\end{exmp}
...
\begin{exmp}\label{VNR}
A ring $R$ is \textit{von Neumann regular} if for any $r\in R$, there exists $x_r\in R$ such that $rx_rr=r$. 

Any one-sided ideal in $R$ is generated by an idempotent element. Let $\faktor{E}{\sim}$ denote the set of idempotent elements in $R$ modulo the equivalence relation where $e_1 \sim e_2$ if the two generate the same ideal. Then  $\faktor{E}{\sim}$  forms a lattice where the join and meet correspond to the addition and intersection of ideals, respectively.

 Kaplansky (1998) proved that any projective $R$-module is a direct sum of $\left(e\right)$ with $e$ idempotent. It follows that $\faktor{E}{\sim}$ determines $K_0(R)$.
\end{exmp}
...
If $I\subset R$ is a nilpotent ideal, then $\P\left(\faktor{R}{I}\right) \cong \P(R)$, hence $K_0(R) \cong K_0\left(\faktor{R}{I}\right).$
\end{lemma}
...
\begin{definition}
An $R$-module $M$ is a \textit{componentwise free module} if $R = \prod_{i=1}^n R_i$ and $M \cong \prod_{i=1}^n R_i^{c_i}$ for some integers $c_i$.
\end{definition}
...
\end{theorem}
\begin{proof}
We see that $R_{\red}$ is a commutative von Neumann regular ring by \cref{Krull}. Any ideal $\left(d\right)$ in $R_{\red}$  where $d$ is idempotent is componentwise free. By \cref{VNR}, every object $X$ of $\P(R)$ is therefore componentwise free. Therefore, 
\begin{gather*}
...
Our results thus far can be extended to \textit{symmetric monoidal} categories because these come equipped with a notion of direct sum enabling our Grothendieck construction. 

\begin{definition} Let $S$ be a category equipped with a functor $\otimes : S \times S \to S$ and a \textit{unit} object $1$.
\begin{enumerate}
\item We say that $\left(S, \otimes, 1\right)$ is \textit{monoidal} if it is equipped with  natural isomorphisms 
\begin{gather*}
\alpha : \left({-}\otimes {-}\right)\otimes {-} \overset{\cong}{\longrightarrow} {-}\otimes \left({-}\otimes {-}\right)
\\ \lambda : 1 \otimes {-} \overset{\cong}{\longrightarrow} {-} \  \quad \ \rho : {-} \otimes 1 \overset{\cong}{\longrightarrow} {-}
\end{gather*}
ranging over objects in $S$ such that the following two diagrams commute:
\[
\begin{tikzcd}[row sep=large]
\left(x \otimes 1\right)\otimes y \arrow[rd, "\rho_x\otimes \id_y"'] \arrow[rr, "{\alpha_{x,1,y}}"]                                           &                                                                                                & x\otimes \left(1\otimes y\right) \arrow[ld, "\id_x\otimes \lambda_y"]                               \\
                                                                                                                                              & x\otimes y                                                                                     &                                                                                                     \\
                                                                                                                                              & \left(w\otimes x\right)\otimes \left(y\otimes z\right) \arrow[rd, "{\alpha_{w,x,y\otimes z}}"] &                                                                                                     \\
\left(\left(w\otimes x\right) \otimes y\right)\otimes z \arrow[ru, "{\alpha_{w\otimes x, y, z}}"] \arrow[d, "{\alpha_{w,x,y}\otimes \id_z}"'] &                                                                                                & w \otimes \left(x\otimes\left( y\otimes z\right)\right)                                             \\
\left(w \otimes \left(x\otimes y\right)\right)\otimes z \arrow[rr, "{\alpha_{w, x\otimes y, z}}"']                                            &                                                                                                & w \otimes \left(\left(x\otimes y\right)\otimes z\right) \arrow[u, "{\id_w\otimes \alpha_{x,y,z}}"']
\end{tikzcd}.
\]
(This is called the \textit{pentagon identity}.)
\item We say that $\left(S, \otimes, 1\right)$  is \textit{symmetric monoidal} if it is monoidal and is equipped with a \textit{braiding} \linebreak $B_{x,y} : x\otimes y \to y\otimes x$ for all $x,y\in \ob{S}$ such that 
\[
B_{y,x}\circ B_{x,y} = \id_{x\otimes y}
\] and the hexagon 
\[
\begin{tikzcd}[row sep=large]
                                                                       & \left(x\otimes y\right)\otimes z \arrow[rd, "{\alpha_{x,y,z}}"] \arrow[ld, "{B_{x,y}\otimes \id_z}"'] &                                                                  \\
\left(y\otimes x\right)\otimes z \arrow[d, "{\alpha_{y,x,z}}"']        &                                                                                                       & x\otimes \left(y\otimes z\right) \arrow[d, "{B_{x,y\otimes z}}"] \\
y\otimes \left(x\otimes z\right) \arrow[rd, "{\id_Y\otimes B_{x,z}}"'] &                                                                                                       & \left(y\otimes z\right)\otimes x \arrow[ld, "{\alpha_{y,z,x}}"]  \\
                                                                       & y\otimes\left(z\otimes x\right)                                                                       &                                                                 
\end{tikzcd}
\] commutes.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
\item Any $k$-vector space where $\otimes \equiv \otimes_k$ and $e \equiv k$.
\item The category of pointed topological spaces where $s \otimes t \equiv s \wedge t $ and $e \equiv S^0$.
\item Any category with finite coproducts where $s \otimes t \equiv s \amalg t$ and $e \equiv 0$.
\item Any category with finite products where $s\otimes t \equiv s \times t$ and $e \equiv \ast$.

Such a monoidal category is called \textit{cartesian}. Any monoidal category whose unit is a terminal object is called \textit{semicartesian}.
\end{enumerate} 
\end{exmp}

Suppose that the class of isomorphism classes of objects of a category $S$ is a set and denote it by $S^{\iso}$. If $S$ is symmetric monoidal, then $\left(S^{\iso}, \otimes \right)$ is an abelian monoid with identity element $e$. In this case, we define the \textit{Grothendieck group of $S$} as $K_0(S)$.
