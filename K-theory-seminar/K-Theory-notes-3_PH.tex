...
\begin{exmp}
Suppose that $R$ is commutative. There is a ring homomorphism $R\to F$ with $F$ a field. Then the induced map $K_0(R) \to K_0(F) \cong \Z$ sends $[R]$ to $1$. Also, the map $\phi : \Z \to K_0(R)$ is injective by Lemma 4. Letting $K\coloneqq \ker(K_0(R) \to \Z)$, we get a split exact sequence of abelian groups, so that $K_0(R) \cong \Z \oplus K$.
\[
\begin{tikzcd}
