...
\] where  $\lvert{Q}\rvert = m$, $\Sigma$ is an alphabet, $q_0\in Q$, $\delta : Q\times \Sigma \to Q$, and $Q_F\subset Q$. We call $\delta$ the \textit{transition function of $M$}.
...
\end{theorem}
\begin{proof}
Let $N = \left(Q, \Sigma, q_0, \delta_N, Q_F\right)$. Define the $\DFA$ $$M \coloneqq \left(\P(Q), \Sigma, q_0^{(1)} , \delta_M, Q_F^{(1)} \right)$$ where $q_0^{(1)} \equiv \{q_0\}$, $\delta_M (Q', \sigma) \equiv \bigcup_{\gamma \in Q'} \delta_N(\gamma, \sigma)$ and $Q_F^{(1)} \equiv \left\{R \subset Q : R\cap Q_F \ne \emptyset\right\}$. For any string $x$, one can use induction on $\lvert{x}\rvert$ to show that if $R\subset Q$, then $$\tilde{\delta}_M(R, x) = \bigcup_{p\in R} \hat{\delta}_N(p,x)$$ where $\tilde{\delta}_M : \P(Q) \times \Sigma^{\ast} \to \P(Q)$ denotes the obvious extension of $\delta_M$ to strings.  By setting $R= \{q_0\}$, we are done.
\end{proof}
...
\item $\lvert{wy}\rvert\leq n_0$, and
...
\end{lemma}
\begin{proof}
By assumption, there is some $\DFA$ $M = (Q, \Sigma, \delta, q_0, Q_F)$ such that $L = L(M)$. Let $n_0 = \lvert{Q}\rvert$. Let $x\in L$ such that $\lvert{x}\rvert\geq n_0$. Set $q_i = \hat{\delta}(q_0, x_0 \cdots x_i)$ for each $i\geq 0$.  There exist $0\leq i<j \leq n_0$ such that $q_i = q_j$. Consider the three strings 
\begin{gather*}
w\coloneqq x_1\cdots x_i 
\\  y\coloneqq  x_{i+1}\cdots x_j 
\\  z\coloneqq x_{j+1} \cdots x_m.
\end{gather*}
where $\lvert{x}\rvert = m$. It is straightforward to verify that these satisfy conditions (a), (b), and (c).
\end{proof}
\begin{corollary} $ $
\begin{enumerate} 
\item Our last proof shows that any regular language $L$ has pumping length $\leq \lvert{Q}\rvert$.
\item If $L(M) \ne \emptyset$, then there exists $x\in L(M)$ such that $\lvert{x}\rvert \leq \lvert{Q}\rvert$.
\end{enumerate}
\end{corollary}

\begin{exmp} Let $n_0\geq 0$ be an integer. Suppose that $x=wyz$ with $\lvert{y}\rvert >0$ and $\lvert{wy}\rvert\leq n_0$.
\begin{enumerate}
\item Let $L=  \left\{ 1^{2^n} : n\geq 0\right\}$. Let $x= 1^{2^{n_0+1}}$.  Note that $$\lvert{wy^iz}\rvert = \lvert{wyz}\rvert + (i-1)\lvert{y}\rvert = 2^{n_0+1} + (i-1)\lvert{y}\rvert.$$ Hence if $i=2$, then $\lvert{wy^i z}\rvert = 2^{n_0+1} +\lvert{y}\rvert \leq 2^{n_0+1} +n_0 < 2^{n_0+2}$ since $n_0 < 2^{n_0+1}$, in which case $2^{n_0+1} < \lvert{wy^i z}\rvert$ as well. Therefore, $L$ is not a regular language. 
\item Let $L = \left\{ ww : w \in \left\{0,1\right\}^{\ast}\right\}$. Let $x= 0^{n_0}10^{n_0}1$. If $\lvert{y}\rvert= m$ with $0<m\leq n_0$,  then $wz= 0^{n_0-m}10^{n_0}1$, which does not belong to $L$. Hence $L$ is \emph{not} a regular language. 
\end{enumerate}
\end{exmp}
...
Given any $\TM$ $M$, its \textit{time complexity} is the function $f: \N \to \N$ where $$f(n) \equiv\max\{\text{steps used by }M \text{ on input }w\mid \lvert{w}\rvert =n\}.$$ We say that $M$ \textit{runs in time $f(n)$}.
\end{definition}
...
\begin{lemma}
A language $L$  belongs to $\NP$ if and only if there exist  a deterministic $\TM$ $V(\cdot, \cdot)$ and constants $c_1, c_2 >0$ such that $$L = \left\{ x \mid \exists y.\lvert{y}\rvert \leq \lvert{x}\rvert^{c_1} \land V(x,y) = 1 \text{ where } V \text{ runs in time }\lvert{x\cdot y}\rvert^{c_2}\right\}.$$ In this case, we call $V$ a \textit{verifier} and $y$ a \textit{witness}.
\end{lemma}
\begin{proof} $ $
...
Let $M= \left(Q, \Sigma, \Gamma, q_0, \delta, Q_F, Q_R\right)$ be a $\TM$. Suppose that on any input of size $n$, $M$ halts in at most $t$ steps with $t\geq n$. Then there is a circuit of size $O(t^2 \cdot (\lvert{\Gamma}\rvert\cdot \lvert{Q}\rvert)^3) = O(t^2)$ that  outputs $1$ on a  string $x$ of length $n$ if and only if $M$ accepts $x$.
\end{lemma}
\begin{proof}
We want to encode a given configuration of $M$, which we may assume uses at most $t$ cells of tape. To do this, we take $\log{\lvert{\Gamma}\rvert}$ bits, $1$ bit, and $\log{\lvert{Q}\rvert}$ bits to encode the content of the current cell, whether or not the head is located at this cell, and, if so, the current state, respectively.  For each $i,j\geq0$, the bit $b_{j, l+1}$ representing the $j$-th cell at time $l+1$ depends precisely on the three bits $b_{j-1, l}$, $b_{j, l}$, and $b_{j+1, l}$. If $B\coloneqq 1+ \log{\lvert{\Gamma}\rvert} +\log{\lvert{Q}\rvert}$, then every bit of the encoding of the configuration of $M$ at time $l+1$ depends on $3B$ bits. This determines a Boolean function $f: \left\{0,1\right\}^{3B}\to \left\{0,1\right\}$ that computes the next configuration. \Cref{pl1} implies that $f$ be be computed by a circuit of size $O(2^{3B})$ and hence by one of size $O(\left(\lvert{\Gamma}\rvert\cdot \lvert{Q}\rvert\right)^3)$. Thus, there is a circuit of size $O(t\cdot t\cdot  \left(\lvert{\Gamma}\rvert \cdot \lvert{Q}\rvert\right)^3 )$ that simulates $M$ on inputs of size $n$.
\end{proof}
...
\begin{proof}
Let $G$ be a graph with an independent set $S$ with $\lvert{S}\rvert\geq k$.  Then $V \setminus S$ is a vertex cover of $G$. Conversely, if $G$ has a vertex cover $K$ of size at most $\lvert{V}\rvert - k$, then $V \setminus K$ is an independent set of size at least $k$ in $G$. Thus, the algorithm $\langle G, k \rangle \mapsto \langle G, \lvert{V}\rvert - k\rangle$ defines a suitable reduction.  
\end{proof}
\end{enumerate}
\end{exmp}
...
\end{enumerate}
Set $C = \left\{ [u,v] : (u,v) \in E\right\}$. Also, set $W= \lvert{E}\rvert+ k -1$. Let $G' = \left(V', E', w'\right)$. Note that we have constructed $G'$ in polynomial time.
\begin{claim}
$G'$ has a Steiner tree containing $C$ with weight at most $\lvert{E}\rvert + k -1$.
\end{claim}
\begin{proof}
...
\end{align*} Since $S$ is a vertex cover, we see that $(S', E_{S'})$ is a connected subgraph of $G'$ that contains $C$ and has weight $\lvert{E}\rvert+k -1$.
\end{proof}
\begin{claim}
If $C$ has a a Steiner tree $T$ of total weight $W \leq \lvert{E}\rvert +k -1$, then $G$ has a vertex cover of size $k$. 
\end{claim}
\begin{proof}
Alter $T$ as follows.
\begin{itemize}
...
\end{itemize}
The resultant graph $T'$ is connected and contains $C$. Note that $T'$ has weight at most $\lvert{E}\rvert + k -1$ where each edge of $T'$ has weight $1$. This implies that $T'$ spans at most $\lvert{E}\rvert +k$ vertices. Since $T'$ contains $C$, it follows that $T'$ contains a set $R$ of vertices of the form $[v]$ such that $\lvert{R}\rvert\leq k$. If $(x,y)\in E$, then $[x,y]$ is connected to some $[v_0]$ by edges of weight $1$. This means that $[x]$ or $[y]$ belongs to $T'$, so that $[x]$ or $[y]$ belongs to $R$. This shows that $R$ is a vertex cover for $G$.
\end{proof}
\end{proof}
...
\end{exmp}
\begin{proof}
It's clear that this belongs to $\NP$. We show that $\mathsf{VC} \leq_m^p \mathsf{SUBSET{-}SUM}$. Let $G=\left(V, E\right)$ such that $\lvert{V}\rvert = n$ and $\lvert{E}\rvert= m$. We can make $E$ totally ordered. Suppose that $G$ has a vertex cover $C$ of size $k$. For each $v\in V$, we define an integer $a_v\geq 0$ in base-4 (written from left to right) consisting of $m+1$ digits. Further, for each $e\in E$, we define an integer $b_e\geq 0$ in base-4 consisting of $m+1$ digits. Specifically, if $0\leq i \leq \lvert{E}\rvert-1$ and  $(u,v) \in E$ is the $i$-th edge, then define both $a_u$ and $a_v$  as the integer $$0 \cdots 0 \underbrace{1}_{i\text{-th digit}} 0 \cdots 01   $$ and define $b_{(u,v)}$ as the integer $$0 \cdots 0 \underbrace{1}_{i\text{-th digit}} 0 \cdots 00     .$$ Now, set $t= k \cdot 4^m + \sum_{i=0}^{m-1}2 \cdot 4^i$.
...
\begin{prop}
A language $L$ belongs to $\mathbf{NL}$ if and only if there exist $c\in \N$ and a deterministic $\TM$ $V(\cdot, \cdot)$ consisting of one read-only input tape, one work tape, and one read-only, single-axis proof tape  such that $V$'s work tape uses $O(\log{\lvert{\text{(first input)}}\rvert})$ space and $$L = \left\{ x \mid \exists y.\lvert{y}\rvert \leq \lvert{x}\rvert^{c} \land V(x,y) = 1\right\}.$$ .
\end{prop}
...
Finally, let $c\in \N$. Given that $\lvert{C_{i-1}}\rvert=k$, we can verify that $\lvert{C_i}\rvert = c$ as follows. 
\begin{enumerate} 
...
\begin{definition} $ $
A language $L$ belongs to $$\underbrace{\mathsf{BPTIME}(t(n))}_{\textit{bounded error probabilistic time}}$$ if there exists a randomized $\TM$ $M$ running in time $t(\lvert{x}\rvert)$ (with probability $1$) such that $M$ decides $L$ with respect to $t(n)$. Let $\mathbf{BPP}= \bigcup_{c\geq 0} \mathsf{BPTIME}(n^c)$.
\end{definition}
...
\item Let $p(x)$ be a univariate polynomial of degree $d \geq 0$. Let $S\subset \Z$ be finite. Then $$\mathbb{P}\left[p(x) =0\right] \leq \frac{d}{\lvert{S}\rvert}$$ for any $x\in_R S$.
\end{enumerate}
\end{prop}

\begin{lemma}[DeMillo-Lipton-Schwartz-Zippel]\label{Dem}
Let $p(x_1, \ldots, x_n)$ be a multivariate polynomial of degree at most $d\geq 0$. Let $S \subset \Z$ be finite. Then for any elements $a_1, \ldots, a_n$ randomly chosen with replacement from $S$, $$\mathbb{P}\left[p(a_1, \ldots, a_n) =0\right] \leq \frac{d}{\lvert{S}\rvert}.$$
\end{lemma}
\begin{proof}
...
\begin{exmp}
Let $G=\left(V_1, V_2, E\right)$ be a bipartite graph with $\lvert{V_1}\rvert=\lvert{V_2}\rvert=n$. 
...
$\mathbb{P}\left[\lvert{Z - \mu}\rvert \geq t\right] \leq e^{\frac{{-t^2n} \mu}{4}}$ for any $t\in (0,1)$.
\end{lemma}

\begin{corollary}
Let $M$ be a randomized polynomial-time $\TM$ and $L\subset \Sigma^{\ast}$ be a language. Suppose that there exists $c>0$ such that
\begin{itemize}
\item for any $x\in L$, $\mathbb{P}_{r\in_R \left\{0,1\right\}^{\poly(\lvert{x}\rvert)}}\left[M(x,r)=1\right] \geq \frac{1}{2} +n^{-c}$ and
\item for any $x\notin L$, $\mathbb{P}_{r\in_R \left\{0,1\right\}^{\poly(\lvert{x}\rvert)}}\left[M(x,r)=0\right] \geq \frac{1}{2} + n^{-c}$
\end{itemize} where $n$ denotes $\lvert{x}\rvert$.
Then for any $c' >0$, there exist a randomized polynomial-time $\TM$ $M'$ such that 
\begin{itemize}
\item for any $x\in L$, $\mathbb{P}_{r\in_R \left\{0,1\right\}^{\poly(\lvert{x}\rvert)}}\left[M'(x,r)=1\right] \geq 1 - 2^{{-}n^{c'}}$ and
\item for any $x\notin L$, $\mathbb{P}_{r\in_R \left\{0,1\right\}^{\poly(\lvert{x}\rvert)}}\left[M'(x,r)=0\right] \geq 1 - 2^{{-}n^{c'}}$.
\end{itemize}
\end{corollary}
\begin{proof}
...
\begin{enumerate}
\item $\lvert{\aut(G \cup H)}\rvert \geq 2k$ whenever $G \cong H$  and 
\item $\lvert{\aut(G \cup H)}\rvert \leq k$ whenever $G \not\cong H$. 
\end{enumerate}
\end{enumerate}
\end{prop}
...
\item it is guaranteed that either $\lvert{S}\rvert \leq \frac{K}{2}$ or $\lvert{S}\rvert \geq K$.
\end{itemize}
To do this, choose $l\in \N$ such that $2^{l-2} \leq K \leq 2^{l-1}$ and $\lvert{S}\rvert \leq 2^{l-1}$. Let $\H$ denote a pairwise independent set of mappings $\left\{0,1\right\}^m \to \left\{0,1\right\}^l$. 
...
\begin{caseof}
\case{Suppose that $\lvert{S}\rvert \leq \frac{K}{2}$.}{If $x \in S$, then $\mathbb{P}_{h \in_R \H}[h(x) = y] = \frac{1}{2^l}$. Moreover, if $p$ denotes the quantity $\frac{\lvert{K}\rvert}{2^l}$, then $$ \mathbb{P}_{h \in_R \H}[\exists x \in S, \ h(x) = y] \leq \frac{\lvert{S}\rvert}{2^l} \leq  \frac{\lvert{K}\rvert}{2 \cdot 2^l} = \frac{p}{2} . $$}
\case{Suppose that $\lvert{S}\rvert \geq K$.} {We compute 
\begin{align*}
...
\\ & \geq \frac{\lvert{S}\rvert}{2^l} - \underbrace{\frac{\lvert{S}\rvert(\lvert{S}\rvert-1)}{2}}_{\lvert{S}\rvert \choose 2}\cdot \frac{1}{2^{2l}}
\\ & = \frac{\lvert{S}\rvert}{2^l} \left(  1- \frac{\lvert{S}\rvert -1}{2} \cdot \frac{1}{2^l}    \right)
\\ & \geq \frac{\lvert{S}\rvert}{2^l} \left(  1 - \frac{\lvert{S}\rvert}{2 \cdot 2^l}    \right)
\\ & \geq \frac{K}{2^l} \left(  1 - \frac{\lvert{S}\rvert}{2 \cdot 2^l}    \right)
\\ & \geq \frac{3}{4}\cdot p
.  \end{align*}}
\end{caseof}
\end{exmp}
