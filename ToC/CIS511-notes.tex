...
\begin{enumerate}[label=(\alph*)]
\item Given a binary string $x$, run $M$ on $\left(\left\langle M_x \right\rangle, \left\langle M_x \right\rangle \right)$ where $M_x$ denotes the $\TM$ corresponding to $x$.
...
\begin{algorithm}[H]
    ...
    \While{the current state is a non-halting state}
      {
        run $U_{\TM}$ on $\left(\left\langle M_1 \right\rangle, w\right) $ for $T$ steps\;
        \eIf{$U_{\TM}$ accepts}{accept}{run $U_{\TM}$ on $\left(\left\langle M_2 \right\rangle, w\right)$ for $T$ steps \;
...
Let $A_{\text{HALT}} = \left\{\left\langle M, w \right\rangle \mid M \text{ halts on }w\right\}$. Then $A_{\text{HALT}}$ is undecidable.
\end{exmp}
\begin{proof}
Recall that $A_{\TM}$ is undecidable. Thus, it suffices to show that $A_{\TM} \leq_m A_{\text{HALT}}$. To do this, we want to design a computable function that maps any $\left\langle M, w \right\rangle$ to another $\left\langle M' \right\rangle, w'$ such that $M'$ halts on $w'$ precisely when $M$ accepts $w$. Construct such an $M'$ as follows.
...
\begin{algorithm}[H]
    \SetKwInOut{Input}{Input}
    \Input{the string $x$}
    {run $U_{\TM}$ on $\left(\left\langle M \right\rangle, x\right)$}\;
...
Then we get a suitable function given by $\left(\left\langle M \right\rangle , w\right) \mapsto \left(\left\langle M' \right\rangle, w\right)$.
\end{proof}
...
\begin{exmp}
Let $\mathsf{SUBSET{-}SUM} = \left\{\left\langle a_1, \ldots, a_k, t\right\rangle \mid a_i,t\geq 0,\ \exists S \subset [k].\sum_{i\in S} a_i = t\right\}$. Then $\mathsf{SUBSET{-}SUM}$ is $\NP$-complete.
\end{exmp}
\begin{proof}
...
\\ & = O\left(S^2(n)\right) + O(S(n)) = O\left(S^2(n)\right)  .
\end{align*}
\end{proof}
