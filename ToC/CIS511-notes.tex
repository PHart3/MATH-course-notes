...
\item $E_L$ prints $w_j$ whenever $N$ accepts it.
\end{enumerate}

It is easy to see that $E_L$ enumerates $L$.
...
\\ \infty & \text{otherwise}
\end{cases}.
\] To obtain an absolute definition of \textit{Kolmogorov complexity}, let $n_f$ denote the length of $\langle f\rangle$ and define a new partial computable function $g: \left\{0,1\right\}^{\ast} \to\left\{0,1\right\}^{\ast}$ by
\[
0^{n_f}1\langle f\rangle y \mapsto U_{\TM}(\langle f\rangle, y).
...
Let $A_{\text{HALT}} = \left\{\langle M, w \rangle \mid M \text{ halts on }w\right\}$. Then $A_{\text{HALT}}$ is undecidable.
\end{exmp}
\begin{proof}
