...
\item There is some $k\in \N$ such that  $|\aut(G \cup H)| \geq 2k$ whenever $G \cong H$  and $|\aut(G \cup H)| \leq k$ whenever $G \not\cong H$. 
\end{enumerate}
\end{prop}
...
\begin{caseof}
\case{Suppose that $|S| \leq \frac{K}{2}$.}{If $x \in S$, then $\mathbb{P}_{h \in_R \H}[h(x) = y] = \frac{1}{2^l}$. Moreover, $$ \mathbb{P}_{h \in_R \H}[\exists x \in S, \ h(x) = y] \leq \frac{|S|}{2^l} \leq  \frac{|K|}{2 \cdot 2^l} = \frac{p}{2}  $$ where $p\coloneqq \frac{|K|}{2^l}$.}
...
\\ & \geq \frac{K}{2^l} \big (  1 - \frac{S}{s \cdot s^l}    \big )
\\ & \geq \frac{3}{4}\cdot p
.  \end{align*}}
\end{caseof}
\end{exmp}
...
\begin{center}
{\textbf{(Final exam review ression)}} 
\end{center}
