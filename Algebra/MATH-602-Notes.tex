...
\in G$. Then $\faktor{G}{R}: G \to \aut\left(\faktor{S}{R}\right)$ given by $g \mapsto ([s] \mapsto [gs])$ is an action.
\end{exmp}
...
Consider 
\begin{align*}
\Q[\tilde{\beta}] & \coloneqq  \Q[\beta_1, \ldots, \beta_n] = \left\{F(\beta_1, \ldots, \beta_n) : F \in \Q[x_1, \ldots, x_n]\right\} \\ \gal(f) & \coloneqq  \left\{\sigma \in S_n : \exists\text{ bijection } g : \Q[\tilde{\beta}] \to \Q[\tilde{\beta}] \text{ s.t. }  g(F(\beta_1, \ldots, \beta_n)) = F(\beta_{\sigma(1)}, \ldots, \beta_{\sigma(n)}) \text{ for all } F \in \Q[x_1, \ldots, x_n]\right\}.
\end{align*}
...
\begin{exercise}[Burnside's lemma] If $G$ and $S$ are finite and $\theta : G \to \aut(S)$ is an action, then for each $g\in G$, consider $\fix(g) \subset S$. Check that $$|G \diagdown S| = \frac{1}{|G|} \sum_g |\fix(g)|.$$ As a hint, consider $\{(g,x): g \cdot x=x\} \subset G \times S$.
\end{exercise}
...
\begin{definition}
Let $G$ be a group with $|G|= p^km$, $p$ prime, $k\geq 1$, $m\geq 1$, and $\left(p,m\right) =1$. Then $H \leq G$ is called a \textit{$p$-Sylow subgroup of $G$} if $|H| = p^k$.
\end{definition}
...
\begin{proof}
If $G$ is simple, then we're done. So assume otherwise. Let $n\in \N$ be maximal so that there is some proper $H \lhd G$ that contains $n$ generators of $G$. Let $S$ denote the set of such $H$. Note that $S$ satisfies the hypotheses of Zorn, giving a maximal element $H'$. Then $\faktor{G}{H'}$ is simple. \reversemarginpar{\marginpar{\small{How do we proceed if $H'$ is not simple or finitely generated? If $G$ is abelian, then we're good, but not otherwise.}}}
\end{proof}
...
\end{lemma}
\begin{proof}
If the first condition holds so that $\sigma = (i_1 \cdots i_r)(j_1 \cdots j_r)\cdots$ with $r$ odd, then construct odd $\tau = (i_1 \; j_1)(i_2 \; j_2) \cdots (i_r \; j_r)$. Note that 
\begin{align*} 
...
\end{proof}
We have three cases to consider. If $p=2$, then $\sigma$ is the product of two independent transpositions. By the lemma, it follows that $N = A_5$. If $p=3$, then $N$ contains all $3$-cycles because any two $3$-cycles are conjugate in $A_5$. Finally, suppose $p=5$. Let $\sigma = (i_1 \cdots i_5)$ and $\tau = (i_1 \; i_2\; i_3) \sigma (i_1 \; i_2\; i_3)^{-1} = (i_2 \; i_3 \; i_1 \; i_4 \; i_5)$. Then $\tau \sigma^{-1} = (i_1 \; i_2 \; i_3) \in N$, implying that $N$ contains all $3$-cycles. In conclusion, $N$ cannot be proper.  \reversemarginpar{\marginpar{\small{Why did we need that whole lemma?}}}
\end{proof}
...
Suppose $G$ has ACC and DCC, so that $G$ is a product of indecomposables \begin{align*}
...
\end{lemma}
\begin{proof}
For each $n\in \N$, set $K_n = \ker \phi^n$ and $H_n = \im \phi^n$. This gives the normal series \begin{align*}
...
We finally return to the proof of Krull-Schmidt. Suppose $r=1$. Let $p_i : G \to A_i$ and $q_j : G \to B_j$ be projections and $f_i : A_i \hookrightarrow G$ and $g_j : B_j \hookrightarrow G$ be inclusions. Note that each $g_j \circ q_j$ is normal and that $\sum_{j=1}^t g_j \circ q_j = \id_G$. Note also that $p_i \circ f_i = \id_{A_i}$ and $q_j \circ g_j = \id_{B_j}$. This gives $$\id_{A_1} = p_1 \circ \id_G \circ f_1 = p_1 \circ \left(\sum_{j=1}^t g_j \circ g_j\right) \circ f_1 = \sum_{j=1}^t (p_1 \circ g_j \circ q_j \circ f_1).$$ Each $p_1 \circ g_j \circ q_j \circ f_1$ is normal, and each sub-sum of $\sum_{j=1}^t (p_1 \circ g_j \circ q_j \circ f_1)$ is normal. Hence if each sub-sum is nilpotent, then our previous lemma implies that $A_1$ is trivial, contrary to the fact that $A_1$ is indecomposable. Hence $ p_1 \circ g_j \circ q_j \circ f_1$ for some $1\leq j \leq t$. Reindex the $B_i$'s so that $B_j = B_1$. 
...
\item Similarly, if $q: G \to K$ is a quotient map, then we get an induced filtration on $K$ given by $$F^aK\coloneqq  q(F^aG) = \faktor{F^aG}{F^aG \cap \ker q}.$$  \reversemarginpar{\marginpar{\small{How does this define a series of subgroups?}}}
\end{enumerate}
\end{note}


Suppose that $F^{\bullet}G$ and $T^{\bullet}G$ are two filtrations on $G$. Define the \textit{graded $i$-th piece} as $$\gr_F^i(G) = \faktor{F^iG}{F^{i+1}G}.$$ By our \cref{filt}, there is an induced filtration $T^{\bullet}\gr_F^i(G)$. Similarly, there is an induced filtration $F^{\bullet}\gr_T^j(G)$. Then we get graded pieces $\gr_T^j\gr_F^iG$ and $\gr_F^i \gr_T^j G$. These produce two \textit{bigraded} groups $\gr_F\gr_TG$ and $\gr_T\gr_FG$.


\begin{lemma}[Zassenhaus or butterfly]
Suppose $G$ is a group with $A \unlhd \widetilde{A} \leq G \geq \widetilde{B} \unrhd B$. Then we have a group isomorphism $$\faktor{A\cdot \left(\widetilde{A} \cap \widetilde{B}\right)}{A \cdot \left(\widetilde{A} \cap B\right)} \cong \faktor{B \cdot \left(\widetilde{A} \cap \widetilde{B}\right)}{B\cdot (A \cap \widetilde{B})}.$$
\end{lemma}
\begin{proof}
...
Then $D\coloneqq  ({A} \cap \widetilde{B}) \cdot \left(\widetilde{A} \cap {B}\right) \cong  \left(\widetilde{A} \cap {B}\right) \cdot ({A} \cap \widetilde{B})$ is normal in $\widetilde{A} \cap \widetilde{B}$. Let $x \in B \cdot \left(\widetilde{A} \cap \widetilde{B}\right)$ and write $x = bc$. Take $cD \in \faktor{\widetilde{A} \cap \widetilde{B}}{D}$. The map $f: x \mapsto cD$ is well-defined. Indeed, if $x = b_1c_2 = b_2c_2$, then $b_2^{-1}b_1 = c_2c_1^{-1}$, so that $c_2c_1^{-1} \in B \cap \widetilde{A} \cap \widetilde{B} = \widetilde{A}\cap B \leq D$, i.e., $c_2D = c_1D$.
...
\end{align*} Therefore, $\faktor{B \cdot \left(\widetilde{A} \cap \widetilde{B}\right)}{ B \cdot (A \cap \widetilde{B})} \cong \faktor{\widetilde{A} \cap \widetilde{B}}{D}$. 
...
\end{corollary}
\begin{proof}
Since each intermediate term in a composition series is a maximal normal subgroup, neither series admits a proper refinement. Hence any refinement must be identical to the original series, and we are done by \cref{schr}.
\end{proof}
...
\begin{definition} Let us define a certain operation on $\ext(K, (A, \alpha))$.
\begin{enumerate}
...
\end{lemma}
\begin{proof}
Injectivity follows from the fact that $i$ is injective. Recall the group law on $G \ltimes_{\beta \circ q} B$ is given by $$(g_1, b_1)(g_2, b_2) = \left(g_1g_2, b_1(\beta \circ q(g_1)(b_2))\right).$$ To see that $A$ is normal, we compute
\begin{align*}
...
\\ &  = (gag^{-1}, b\beta \circ q(g)(\psi(a)) \beta (q(g)\underbrace{q(a)}_{1}q(g^{-1}))(b^{-1}))
...
\end{tikzcd},
\]
where $B \cong \faktor{A\times B}{(i\times \psi)(A)}$ via  $b \mapsto [(e, b)]$. Let $\psi_{\ast}$ denote the induced map $G \to G_{\psi_{\ast} \xi}$. Define the extension $\psi_{\ast}\xi$ as the bottom row.
...
1 \arrow[r] & A\times A \arrow[r] \arrow[d, "\text{mult}"'] \arrow[u, equal] & \left(G_{\xi} \times G_{\eta}\right) \times_{K\times K} K \arrow[r] \arrow[u] \arrow[d] & K \arrow[r] \arrow[u, "\Delta"] \arrow[d, equal] & 1 \\
...
\item We call $Z^2(K, (A, \alpha)) \coloneqq  \{f \in C^2 : \eqref{eqn:star} \text{ holds}\}$ the set of \textit{second cocycles of $K$ with coefficients in $(A, \alpha)$}.
\end{enumerate}
\end{definition}
...
\[
\{(\xi, s) : \xi \in \ext(K, (A, \alpha)), \ s \text{ a normalized section}\} \longleftrightarrow Z^2(K, (A, \alpha))_0 = Z^2 \cap C^2_0
.\]


If $\tilde{s}$ and $s$ are both normalized sections, then $c(x)\coloneqq \tilde{s}(x)s(x)^{-1}$ is a map $c : K \to A$ such that $c(e) =e$. Let $\tilde{f}$ be the second cochain obtained from $\tilde{s}$. Then  $$\tilde{f}(x, y) = \tilde{s}(x)\tilde{s}(y)\tilde{s}(xy)^{-1} =c(x)s(x)c(y)s(y)(c(xy)s(xy))^{-1}, $$ and thus 
...
\end{align*}
This gives a map $\delta: C^1_0 \to Z^2_0$ defined by $c \mapsto \left(\delta_c : (x, y) \mapsto   c(x)\alpha_x(c(y))c(xy)^{-1}\right)$, which we call the \textit{first Hochschild differential of $K$ with coefficients in $(A, \alpha)$}.  \reversemarginpar{\marginpar{\small{How do we know any first normalized cochain can be written in that form?}}}
...
\end{tikzcd}
\]
\underline{Case 1:} $\M_2$ acts trivially on $\Z$. Then $C^2_0 = \left\{f : \M_2 \times \M_2 \to \Z \mid f(1, y) = f(x, 1) = 0 \text{ for any } x,y \in \M_2 \right\}$. Each $f \in C^2_0$ is thus determined by $f(-1, -1)$, giving $C^2_0 \cong \Z$ via $f\mapsto f(-1, 1)$.
...
\medskip
 ...
 Moreover, $C^1_0 = \{c : \M_2 \to \Z : c(1) =0\} \cong \Z$, giving the correspondence $\Z \ni b \longleftrightarrow (c: -1 \mapsto b)$. Then the differential $\delta : C^1_0 \to Z^2_0\cong \Z$ is given by $\delta_c(x, y) = c(x)+c(y) -c(xy)$, so that $\delta_c(-1, -1) = c({-}1) + c({-}1) = 2b$. That is, $\delta : \Z \to \Z$ is given by $b \mapsto 2b$. This implies that $HH^2 = \faktor{\Z}{2}$, so that the only nontrivial extension is
\[
\begin{tikzcd}
1 \arrow[r] & \Z \arrow[r, "\text{mult}_2"] & \Z \arrow[r] & \faktor{Z}{2} \arrow[r] & 1
\end{tikzcd}.
\]
\underline{Case 2:} The action is nontrivial with $-1 \mapsto (n \mapsto -n)$.  Again we get $C^2_0 \cong \Z$. Moreover, if $f\in Z_2^0$ and $y= z = -1$, then 
\begin{align*}
...
\begin{remark}
 $|(S)_{\bullet}|$ is homotopy equivalent to $S$ equipped with the discrete topology. 
\end{remark}
...
\end{definition}
The class $\{\Hom_{\c}(y, x)\}_{y\in \ob \c}$ of $y$-points reconstructs $x$ as an object in $\c$. To see this, let $\widehat{\c}\coloneqq  \Fun(\c^{\op}, \mathbf{Set})$ and $x \in \ob \c$. Define the functor $h_x : \c^{\op} \to \mathbf{Set}$ on objects and morphisms, respectively, by $$y \mapsto \Hom_{\c}(y, x) \quad \quad h_x(f) : u \mapsto u \circ f.$$ 
\begin{definition}
...
\item There is a natural isomorphism $$\Hom_{\c}(-, -) \cong \Hom_{\widehat{\c}}\left(h_{(-)}, h_{(-)}\right)$$ of functors $ \c^{\op} \times \c \to \mathbf{Set}$, so that $h : \c \to \widehat{\c}$ is fully faithful. Thus, we can treat objects in $\c$ as set-valued presheaves of $\c$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
Let $F \in \widehat{\c}$. Recall that $F$ is representable by $x$ if there is some isomorphism of functors $h_x \cong F$. By the proof of Yoneda, this is completely determined by $\xi\coloneqq  h_x(\id_x) \in F(x)$. Given $\xi \in F(x)$, we get a natural map \begin{align*}
h_x(y) &  \to F(y)
\\  f &  \mapsto F(f)(\xi).
...
Suppose that $L : \c \to \d$ is a functor. Then $L$ induces a functor $L_{\ast} : \widehat{\d} \to \widehat{\c}$ given by $F \mapsto F \circ L$. We can compose $L_{\ast}$ with the Yoneda functor $h^{\d} : \d \to \widehat{\d}$ to get $L_{\ast} \circ h^{\d} : \d \to \widehat{\c}$. 
...
\end{prop}
\begin{note}[(Co)unit of an adjunction]
 Let $\overset{L}{\underset{R}{\c \rightleftarrows \d}}$ be an adjoint pair of functors. Then there is a natural bijection $$\Hom_{\d}(L(x), L(x)) \cong \Hom_{\c}(x, R \circ L(x))$$ for every $x\in \ob \c$. This yields a map of functors $\epsilon : \id_{\c} \to R \circ L$, called the \textit{unit of the adjunction}. Likewise, we get a map $\eta : L \circ R \to \id_{\d}$, called the \textit{counit of the adjunction}.
In turn, these induce the functors 
\[
\begin{tikzcd}
...
\begin{exercise}
$\id_L \cong \eta \circ \epsilon$.
\end{exercise}

\begin{prop}
Conversely, if $\left(L, R, \epsilon, \eta\right)$ satisfies $\eta \circ \epsilon \cong \id_L$ and $\eta \circ \epsilon \cong \id_R$, then $(L, R)$ is an adjoint pair.
\end{prop}
...
\begin{exmp} $ $
\begin{enumerate}
\item Let $\lvert{\cdot}\rvert : \mathbf{Grp} \to \mathbf{Set}$ denote the forgetful functor. Then it has as left adjoint the free group functor $\Fr : \mathbf{Set} \to \mathbf{Grp}$. This means that $\Hom(\Fr(S), G) \cong \Hom(S, |G|)$ for any set $S$ and group $G$. That is, for any function $f: S \to |G|$, there is a unique homomorphism $\phi : \Fr(S) \to G$ such that $\phi \restriction_S = f$, where we embed $S \hookrightarrow \coprod_{s\in S} G_s$ in $\mathbf{Set}$ by $s\mapsto \underbrace{1_{G_s}}_{\text{generator}}$.
\begin{proof}
\begin{align*}
 \Hom(\Fr(S), G) & = \Hom\left(\coprod_S \Z, G\right) 
 \\ & \cong  \prod_{s\in S} \Hom(\Z, G) 
 \\ & \cong \prod_s \Hom(\{1\}, |G|)
 \\ &  \cong \Hom\left(\coprod_s \{1\}, |G|\right) 
 \\ & \cong  \Hom(S, |G|).
 \end{align*}
\end{proof}
...
$$\{e\} \not \cong \Hom_{\mathbf{Grp}}\big(F(A), \faktor{F(A)}{M}\big) \cong \Hom_{\mathbf{Ab}}\big(A, \left(\faktor{F(A)}{M}\right)^{\ab} \big)\cong \{e\},$$ a contradiction. If $\faktor{F(A)}{M}$ is abelian, then it is isomorphic to $C_p$ and we can make an argument as before.
\end{proof}
\end{enumerate}
\end{proof}
...
\begin{remark} There is another proof if we restrict our set-theoretic universe. Specifically,
the adjunction $(\Fr, \lvert - \rvert)$ yields  
\begin{align*}
P(T) & \cong \Hom_{\mathbf{Set}}(T, C_2)
...
\item A subgroup of a finitely generated group need not be finitely generated. For example, $F_2\coloneqq \Fr(\{x,y\})$ is finitely generated, but the subgroup $\left\{y^kxy^{-k} : k \geq 0\right\}$ is not.
...
\begin{definition}
Let $A \in \mathbf{CommRing}$. Then we have the $A$-module $\bigoplus_{\N} A$ For each $k\geq 0$, define $$m_k = \left(0, \ldots, 0, \underbrace{1}_{k-\text{th place}}, 0 , \ldots \right).$$ Then the $m_k$ form an $A$-basis for  $\bigoplus_{\N} A$. Define $m_k \cdot m_l = m_{k+1}$ and extend this operation to $\bigoplus_{\N} A$ by linearity. Then $\left(\bigoplus_{\N} A, + , \cdot\right) \in  \mathbf{CommRing}$. Moreover, $\left(\left(\bigoplus_{\N} A, + , \cdot\right), i\right) \in  \mathbf{CommRing}^A$ where $i : A \to \bigoplus_{\N}A$ denotes inclusion by $a \mapsto \left(a, 0, \ldots, 0, \ldots\right)$. We call this the \textit{one-variable ring over $A$}.
\end{definition}
...
\begin{definition}
Let $S$ be a set. Note that $\overbrace{\Fun^{\fs}(S, \Z_{\geq 0})}^{\text{finite support}}$ is an additive monoids because $\Z$ is one. View its elements as monomials in elements of $S$. For any $s\in S$, define $t_s : S \to \Z_{\geq 0}$ by $$x \mapsto \begin{cases} 0 & s \ne x \\ 1 & s =x \end{cases}.$$ Then for any $\xi \in \Fun^{\fs}(S, \Z_{\geq 0})$, we write $\xi = \prod_{s\in S}t_s^{\xi(s)} =\prod_{s\in \supp(\xi)}t_s^{\xi(s)}$.
...
\end{tikzcd}
.\]
Since the free module functor is left adjoint to the forgetful functor, it follows that $\beta$ lifts to a homomorphism $ \coprod_{t\in T}R \overset{\theta}{\longrightarrow} M$  if and only if the set map $T \to \faktor{M}{N}$ lifts to a set map $T \to M$. But there is some set-theoretic section $s: \faktor{M}{N} \to M$, making $T \overset{\beta}{\longrightarrow} \faktor{M}{N} \overset{s}{\longrightarrow} M$ such a lift in $\mathbf{Set}$. Thus we obtain such a lift $\theta$ in $R{-}\mathbf{Mod}$. Define the homomorphism $$\phi : \left(\coprod_{s\in S} R\right) \coprod \left(\coprod_{t\in T} R\right)\to M, \quad \quad (x,y) \mapsto \alpha(x) + \theta(y),$$ which satisfies
\[
\begin{tikzcd}
0 \arrow[r] & N \arrow[r, "i", hook] & M \arrow[r, "q", two heads] & \faktor{M}{N} \arrow[r] & 0 \\
...
Find generators $u_1, \ldots, u_m$ for $A$ over $k$. Let $B$ denote subalgebra of $A^G$ generated by $$\left\{S(u_i^k)\right\}_{i=1, \ldots, m,\ k=1, \ldots, |G|}$$ over $k$. For each $i$, observe that $X_{u_i}(x) \in B[x]$ and that $X_{u_i}(u_i) =0$. It follows that $u_i^{|G|}$ can be written as a $B$-combination of $1, u_i, \ldots, u_i^{|G|-1}$.\reversemarginpar{\marginpar{\small{Why can it be written like this?}}} This implies that any monomial of the form $u_1^{s_1}\cdots u_m^{s_m}$ can be written as a $B$-combination of monomials of the form $u_1^{\alpha_1}\cdots u_m^{\alpha_m}$ where each $0\leq \alpha_i <|G|$. We may thus write $$a= \sum_{\alpha\coloneqq  (\alpha_1, \ldots, \alpha_m)}\phi_{\alpha}u^{\alpha}, \quad \alpha_i < |G|, \quad \phi_{\alpha} \in B.   $$ If $a\in A^G$, then $a=S(a) = \sum_{\alpha}S(\phi_{\alpha})S(u^{\alpha}) = \sum_{\alpha} \phi_{\alpha}S(u^{\alpha})$. As each $\alpha_i<|G|$,  the set $\{S(u^{\alpha})\}_{\alpha}$ is finite. Also, $B$ is finitely generated over $k$. As a result, $A^G$ is finitely generated over $k$. 
\end{proof}
...
\begin{proof}
We verify that $\Hom_R(M, -)$ is left exact. Let 
\[
\begin{tikzcd}
0 \arrow[r] & X' \arrow[r, "f"] & X \arrow[r, "g"] & X'' \arrow[r] & 0
\end{tikzcd} 
\] be a short exact sequence of $R$-modules. Apply $\Hom_R(M, -)$ to get a sequence of abelian groups.
\[
\begin{tikzcd}
...
\end{tikzcd}
\] commutes. Define $\psi : M \to X$ by the composition $M \hookrightarrow M\coprod N \overset{f}{\longrightarrow} X$. Then $\psi$ fills \eqref{eqn:eta}.
...
$(\Longrightarrow)$ As $\coprod_{\lambda} M_{\lambda}$ is projective, there is some $R$-module $N$ such that $$\left(\coprod_{\lambda} M_{\lambda}\right) \coprod N \cong M_{\lambda} \coprod  \left(\coprod_{\alpha \ne \lambda} M_{\alpha}\right)\coprod N$$ is free.
\end{proof}
