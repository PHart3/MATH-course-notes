...
Given a simplicial set $X_{\bullet}$, endow each set $X_n$ with the discrete topology. Define the \textit{geometric realization of $X_{\bullet}$} as the space $$\left\lvert{X_{\bullet}}\right\rvert \equiv \coprod_{m\geq 0} \faktor{(X_m \times \Delta^m)}{\sim}$$ where $X_n \times \Delta^n \ni \left(x, y\right)\sim \left(x', y'\right) \in X_m \times \Delta^m$ if $X(f)(y') = y$ and $\Delta_f(x) = x'$ for some morphism $f: [n] \to [m]$.  
