...
\\ &  = (\alpha \circ q(g)(a),  \psi(\alpha \circ q(g)(a))),
 \end{align*} 
which belongs to $\im(i \times \psi)$. {[[Why does $\alpha \circ q(g)(a) = gag^{-1}$ hold?]]}
...
\end{align*}
This gives a map $\delta: C^1_0 \to Z^2_0$ defined by $c \mapsto \left(\delta_c : (x, y) \mapsto   c(x)\alpha_x(c(y))c(xy)^{-1}\right)$, which we call the \textit{first Hochschild differential of $K$ with coefficients in $(A, \alpha)$}. {[[How do we know any first normalized cochain can be written in that form?]]}
