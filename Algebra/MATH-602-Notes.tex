...
\end{exercise}
\begin{proof}
Given $a$, define $f: G^{\op} \to \aut(S)$ by $g \mapsto \left(x \mapsto a(x, g)\right)$. This is a homomorphism. Conversely, given a homomorphism $f: G^{\op} \to \aut(S)$, define $a(x, g) = f(g)(x)$. This is a right group action.
\end{proof}
...
\begin{definition}  Let $S$ be a set and $G$ be a group.
\begin{enumerate}
\item If $\left(S, +\right)$ is an abelian group and $\theta : G \to \aut_+(S)$ is an action, then we call $S$ a \textit{left $G$-module}.
\item If $S$ is also a vector space over a field $k$ and $\theta$ preserves $k$-linearity, then $\theta$ is called a \textit{$k$-linear representation of $G$}.
\end{enumerate}
\end{definition}

\medskip

\begin{exmp}[Permutation representation]\label{PR}
...
\Cref{PR} holds for any action $\theta: G \to \aut(S^k)$ where $k$ is a field. This is called the \textit{regular representation of $G$}.
...
If both $\tilde{s}$ and $s$ are normalized sections, then $c(x)\coloneqq \tilde{s}(x)s(x)^{-1}$ defines a map $c : K \to A$ such that $c(e) =e$. Let $\tilde{f}$ be the second cochain obtained from $\tilde{s}$. Then  $$\tilde{f}(x, y) = \tilde{s}(x)\tilde{s}(y)\tilde{s}(xy)^{-1} =c(x)s(x)c(y)s(y)(c(xy)s(xy))^{-1}, $$ and thus 
...
\end{align*}
This gives us a map $\delta: C^1_0 \to Z^2_0$ defined by $c \mapsto \left((x, y) \overset{\delta_c}{\longmapsto}   c(x)\alpha_x(c(y))c(xy)^{-1}\right)$, known as the \textit{first Hochschild differential of $K$ with coefficients in $\left(A, \alpha\right)$}.  \reversemarginpar{\marginpar{\small{How do we know any first normalized cochain can be written in that form?}}}
...
\begin{align*}
y &  \mapsto \Hom_{\c}(y, x)
\\  f & \mapsto \left(u  \mapsto u \circ f\right).
\end{align*}
...
For any morphism $x \xrightarrow{f} y$ in $\c$, $h(f)$ is a map $h_x \to h_y$, and $h(f)_z : \Hom_{\widehat{\c}}(z, x) \to \Hom_{\widehat{\c}}(z, y)$ is given by $u \mapsto f \circ u$ for any $z\in \ob \c$. In this case, $h(f)_x(\id_x) = f\circ \id_x = f$. This proves that $i$ is a left inverse of $h$.
...
\item A subgroup of a finitely generated group need \emph{not} be finitely generated. For example, $F_2\coloneqq \Fr(\{x,y\})$ is finitely generated, but the subgroup $\left\langle{y^kxy^{-k} : k \geq 0}\right\rangle$ is not.
...
\begin{definition}
A ring $R$ is \textit{Noetherian} if every ideal of $R$ has  \acc. It is \textit{Artinian} if every ideal has \dcc.
\end{definition}
...
\item Let $R= \C\left[x_1, x_2, \ldots\right]$. This is not Noetherian, because $$\left(x_1\right) \subsetneq \left(x_1, x_2\right) \subsetneq \cdots   $$ fails to stabilize. But $R$ is an integral domain since $\C$ is one. Thus, not all integral domains are Noetherian.
...
Moreover, a finitely generated module over a general ring $R$ need \emph{not} be finitely presentable. To see this, note that $\C$ is an $R$-module via the action $f \cdot a \equiv f(0)a$. We have a short exact sequence
\[
\begin{tikzcd}
0 \arrow[r] & {\left(x_1, x_2, \ldots\right)} \arrow[r] & R \arrow[r, "\text{ev}_0"] & \C \arrow[r] & 0
\end{tikzcd}
\] of $R$-modules.
...
\end{theorem}
\begin{proof}
Note that $A\left[x\right]$ is an $A$-module since $A$ is a subring. We see that $$A\left[x\right] = \bigcup_{n\geq 0} A\left[x\right]_n, \quad A\left[x\right]_n \coloneqq \left\{f \in A\left[x\right] : \deg f \leq n\right\}.$$
...
\medskip
 ...
 Let $\Omega \unlhd A\left[x\right]$ be an ideal. Then $\Omega \cap A\left[x\right]_n$ is an $A$-submodule in $A\left[x\right]_n$ and thus a finitely generated $A$-module with generators, say, $\alpha_1, \ldots, \alpha_{k_n}$. Let $$\widetilde{\Omega} \coloneqq  \left\{a\in A : \text{$a = 0$ or $\left(\exists f\in \Omega\right)\left(\deg f >0 \land f(x) = ax^r+ O(x^{r-1})\right)$}\right\}.$$
\begin{claim}
...
\item If $R$ is a ring, then the functors $\Hom_R(M, -) : R{-}\mathbf{Mod} \to \Z{-}\mathbf{Mod}$ and $\Hom_R(-, M) :R{-}\mathbf{Mod}^{\op} \to \Z{-} \mathbf{Mod}$ are left exact.
\begin{proof}
...
\end{tikzcd}
\] commutes.
\item $M$ is injective  if and only if $\Hom_R(-, M)$ maps monomorphisms $0\to X' \overset{i}{\longrightarrow} X$ to epimorphisms. That is, $\Hom_R(X, M) \xrightarrow{{-} \circ i} \Hom_R(X', M)$ is surjective, so that for any map $\phi : X' \to M$, there is some map $\psi : X \to M$ such that 
\[
\begin{tikzcd}
 & M &  \\
