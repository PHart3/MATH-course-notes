...
\\ & = \{(g, b) : g \in A\} \geq A\times \{e\} \cong A.\end{align*}
\end{proof}
...
\begin{prop}
Conversely, if $(L, R, \epsilon, \eta)$ satisfies $\eta \circ \epsilon \cong \id_L$ and $\eta \circ \epsilon \cong \id_R$, then $(L, R)$ is an adjoint pair.
\end{prop}
...
\begin{definition}
Let $A \in \mathbf{CommRing}$. Then we have the $A$-module $\bigoplus_{\N} A$ For each $k\geq 0$, define $$m_k = \left(0, \ldots, 0, \underbrace{1}_{k-\text{th place}}, 0 , \ldots \right).$$ Then the $m_k$ form an $A$-basis for  $\bigoplus_{\N} A$. Define $m_k \cdot m_l = m_{k+1}$ and extend this operation to $\bigoplus_{\N} A$ by linearity. Then $(\bigoplus_{\N} A, + , \cdot) \in  \mathbf{CommRing}$. Moreover, $((\bigoplus_{\N} A, + , \cdot), i) \in  \mathbf{CommRing}^A$ where $i : A \to \bigoplus_{\N}A$ denotes inclusion by $a \mapsto (a, 0, \ldots, 0, \ldots)$. We call this the \textit{one-variable ring over $A$}.
\end{definition}
