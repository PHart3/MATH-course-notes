...
\end{definition} 
If $M=A$ and $N= \frak{a}$, then $N$ is $S$-saturated if and only if $\frak{a} = \frak{a}^{ec}$. The localization on modules induces an inclusion-preserving bijection $$S^{-1}\left({-}\right) : \{N \subset M \mid N \text{ is } S\text{-saturated}\} \to \{M \mid M \subset S^{-1}M\}.$$
...
\begin{corollary} If $S \subset A$ is multiplicatively closed, then we get a bijection $$\left\{ \p \in \mathsf{Spec}(A) \mid \p \cap S=\emptyset\right\} \overset{\left({-}\right)^e}{\longrightarrow}\mathsf{Spec}(S^{-1}A).$$  
\end{corollary}
\begin{proof}
...
\begin{corollary}
In particular, the map $$\left(\text{prime ideal of } A \mid A \subset \p\right) \overset{\left({-}\right)^e}{\longrightarrow} \mathsf{Spec}(A_{\p})$$ is a bijection that preserves inclusions of ideals. 
\end{corollary}
