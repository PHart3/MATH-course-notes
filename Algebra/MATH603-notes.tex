...
\end{tikzcd}
.\] 
Then $H^{n+1}(f)([a]) = \left[f(a)\right] = \left[d_B{b}\right] =0$. It follows that $\im{\delta^n} \subset \ker{H^{n+1}(f)}$.

Conversely, let $\left[a\right] \in  \ker{H^{n+1}(f)}$, so that $H^{n+1}(f)([a]) = \left[f(a)\right]= 0$. This means that $f(a) = d_B{b}$ for some $b \in B^n$. Then $\delta^n([g(b)]) = \left[a\right]$. This shows that $\im{\delta^n} \supset \ker{H^{n+1}(f)}$.
\end{proof}
\pagebreak
