...
\begin{remark}{\textbf{(Basic algebraic geometry)}}
...
\begin{theorem}
Let $A$ be local and $M$ an $A$-module. Assume that one of the following conditions holds.
\begin{enumerate}[(a)]
...
\begin{note}{\textbf{(Galois theory)}}
...
\begin{remark}
Finite fields extensions arise naturally from polynomials. 
\end{remark}
...
\begin{definition}
A category $\c$ is \textit{enhanced over $\mathbf{Ab}$} if $\Hom_{\c}(a, b)$ is an abelian group for any $a, b \in \ob{\c}$ and $$\Hom_{\c}(y, z) \times \Hom_{\c}(x,y) \overset{\circ}{\longrightarrow} \Hom_{\c}(x,z)$$ is bilinear for any $x,y,z\in \ob{\c}$.
\end{definition}
...
\begin{definition}
A category $\c$ is called \textit{additive} if it is enhanced over $\mathbf{Ab}$ and has finite products. 
\end{definition}
