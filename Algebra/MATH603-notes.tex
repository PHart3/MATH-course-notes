...
\end{tikzcd}.
\]
where $\frak{a}$ is an ideal in $R$. We have that $\frak{a} = \left(a\right)$. Therefore, the short exact sequence $$0 \to \frak{a} \to R \to \faktor{R}{\frak{a}} \to 0$$ is isomorphic to $0 \to R  \xrightarrow{\mult_a}  R \to \faktor{R}{\frak{a}}\to 0$. Since $M$ is divisible, we know that \linebreak$M \xrightarrow{\mult_a} M \to 0$ is exact.  Apply $\Hom_R(-, M)$ to get the sequence 
\[
\Hom_R(R, M) \xrightarrow{\left({-}\right)\circ \mult_a} \Hom_R(R, M) \to 0,
\] which is isomorphic to $M \xrightarrow{\mult_a} M \to 0$. This shows that $\left({-}\right)\circ \mult_a$ is surjective. It follows that $\varphi$ can be lifted to some $\psi : R \to M$.
\end{enumerate}
\end{proof}
...
\begin{claim}
Let $\Hom_{\mathbf{Ab}}(-, \faktor{\Q}{\Z}) \coloneqq  \left({-}\right)^D$. Note that for any abelian group $A$, we have a canonical homomorphism $\epsilon_A : A \to A^{DD}$ given by $a \mapsto \left(\left[\varphi :A \to \faktor{\Q}{\Z}\right] \to \varphi(a)\right).$ Then $\epsilon_A$ is injective.
\end{claim}
\begin{proof}
...
Note that $\Hom_R(P^k, -)$ is exact for each $k\geq 0$. Therefore, the sequence $$ 0 \to \Hom_R(P^k, N) \overset{f_k}{\longrightarrow} \Hom_R(P^k, T) \overset{g_k}{\longrightarrow} \Hom_R(P^k, M) \to 0   $$ is exact where $f_k \coloneqq  f\circ \left({-}\right)$ and $g_k \coloneqq  g \circ \left({-}\right)$. Letting $d_i \coloneqq  \left({-}\right)\circ \partial_i$, we get \textit{short exact sequences of complexes} constituting the columns of 
\[
\begin{tikzcd}
...
\end{tikzcd}
.\]
By definition, $\ext_R^i(M, N) = \overbrace{\faktor{\ker{d_i}}{\im{d_{i-1}}}}^{\text{for the first column}}$. Since $\Hom_R(-, M)$ is left-exact and $P^1 \overset{\partial_1}{\longrightarrow} P^0 \overset{\epsilon}{\longrightarrow} M$ is exact, we also have the exact sequence $$ 0 \to \Hom_R(M, M) \overset{\left({-}\right) \circ \epsilon}{\longrightarrow} \Hom_R(P^0, M) \overset{d_0}{\longrightarrow} \Hom_R(P^1, M) .  $$ 
...
\[
N \otimes_R \left({-}\right) : R{-}\mathbf{Mod} \to \mathbf{Ab}
\] is exact.
...
We see that $S^{-1}\left({-}\right)$ is a functor which maps each homomorphism $\varphi : M \to N$ to $S^{-1}\varphi : S^{-1}M \to S^{-1}N$ given by $\frac{m}{s} \mapsto \frac{\varphi(m)}{s}$.  It's easy to verify that $S^{-1}\left({-}\right)$ is left adjoint to the pullback functor $h^{\bullet}$.
...
\begin{corollary}
$S^{-1}\left({-}\right)$ is an exact functor.
\end{corollary}
\begin{proof}
...
\end{definition} 
If $M=A$ and $N= \frak{a}$, then $N$ is $S$-saturated if and only if $\frak{a} = \frak{a}^{ec}$. The localization on modules induces an inclusion-preserving bijection $$S^{-1}\left({-}\right) : \left\{N \subset M \mid N \text{ is $S$-saturated}\right\} \to \left\{M \mid M \subset S^{-1}M\right\}.$$
...
\begin{corollary} If $S \subset A$ is multiplicatively closed, then we get a bijection $$\left\{ \p \in\spec(A) \mid \p \cap S=\emptyset\right\} \overset{\left({-}\right)^e}{\longrightarrow}\mathsf{Spec}(S^{-1}A).$$  
\end{corollary}
\begin{proof}
...
\begin{corollary}
In particular, the map $$\left(\text{prime ideal of } A \mid A \subset \p\right) \overset{\left({-}\right)^e}{\longrightarrow}\spec(A_{\p})$$ is a bijection that preserves inclusions of ideals. 
\end{corollary}
...
\begin{definition}
The \textit{Krull dimension} of $A$ is $$ \dim{A} \equiv \sup \left\{ \mathsf{ht}(\frak{m}) \mid \text{maximal } \frak{m} \properideal A \right\}  .$$
\end{definition}
...
\medskip
 ...
 Let $X=\spec(A)$, the set of all prime ideals in $A$, or \textit{spectrum of $A$}.  For any $f\in A$, define the \textit{principal open subset associated with $f$} as $$X_f \equiv \left\{ \p \in X \mid f\notin \p\right\}.$$  Such subsets satisfy 
\begin{enumerate}[label=(\alph*)]
...
\begin{lemma}
The space $X$ is quasi-compact, which means that for any Zariski-open $U \subset X$, any open cover $\left\{U_{\alpha}\right\}$ of $X$ admits a finite subcover of $U$. 
\end{lemma}

\smallskip

The space $X$, however, is \emph{not} Hausdorff in general. 

\begin{exercise}
Let $A= \C[x]$ and $X =\spec(A)$. Show that $X$ is not Hausdorff. 
\end{exercise}
...
\begin{lemma}
The space $X$ is quasi-compact, which means that any open cover $\left\{U_{\alpha}\right\}$ of $X$ admits a finite subcover. 
\end{lemma}
\begin{proof}
...
\begin{exmp}
Suppose that $X$ is a compact Hausdorff space. Let $A = C(X)$, the ring of complex-valued continuous functions on $X$. Consider $\mathsf{Spec}(A)$ equipped with the Zariski topology and its subset $\Max(A) \coloneqq  \left\{\frak{a} \in C(X) \mid \text{$\frak{a}$ maximal}\right\}$ equipped with the subspace topology.
\begin{claim}
The natural map $X \overset{\varphi}{\longrightarrow} {\Max(A)}$ given by $x \mapsto \left\{f\in C(X) \mid f(x) = 0\right\}$ is a homomorphism.
\end{claim}
\begin{proof}
Let $\frak{a}_x = \ker(\ev_x : A \to \C)$.
...
Let $A$ be  a commutative ring and $M$ an $A$-module. Then $M$ defines a subset of $X\coloneqq \spec(A)$, namely $$\supp(M) \equiv \left\{ \frak{b} \in X \mid M_{\frak{b}} \ne 0\right\},  $$ called the \textit{support of $M$}.


\begin{prop} $ $
\begin{enumerate}
\item $\supp(M) \subset V(\ann(M))$ where $\ann(M) \equiv \left\{a\in A \mid \text{$a\cdot m = 0$ for each $m\in M$}\right\}$. 
\begin{proof}
...
\begin{prop}
$M= \left(0\right)$ $\iff$ $\supp(M) = \emptyset$ $\iff$ $\supp(M) \cap \Max(A) = \emptyset$.
\end{prop}
\begin{proof}
It's clear that $M = \emptyset \implies \supp(M) = \left(0\right) \implies \supp(M) \cap \Max(A) = \emptyset$. Hence it suffices to show that  $$\supp(M) \cap \Max(A) = \emptyset \implies M= \left(0\right).$$ 

On the one hand, if $M$ is finitely generated, then $\supp(M) = V(\ann(M))$, so that $\supp(M)$ must contain any maximal idea that contains $\ann(M) \unlhd A$. Thus, the assumption that $\supp(M) \cap \Max(A) = \emptyset$ implies that $\ann(A)$ is not contained in any maximal ideal, meaning that $\ann(M) = A$. This means that $M= \left(0\right)$. 

On the other hand, if $M$ is arbitrary, then $M = \colim_{\alpha} N_{\alpha}$ with each $N_{\alpha} \subset M$ finitely generated. But then $M_{\frak{a}} = \colim_{\alpha} (N_{\alpha})_{\frak{a}}$ because localization is exact.  Since each $N_{\alpha} = \left(0\right)$, it follows that $\colim_{\alpha} N_{\alpha} =0$ as well. 
\end{proof}


\begin{corollary}
If we have a sequence of modules $$\eta: M \overset{f}{\longrightarrow} T \overset{g}{\longrightarrow} N,$$ then $\eta$ is exact at $T$ $\iff$ $\eta_{\frak{p}}$ is exact at $T_{\frak{p}}$ for each $\frak{p} \in\spec(A)$ $\iff$ $\eta_{\frak{a}}$ is exact at $T_{\frak{a}}$ for each $\frak{a} \in \Max(A)$>  
\end{corollary}
\begin{proof}
...
\begin{definition}
Suppose that $\Pi$ is a property of $A$-modules or of morphisms of $A$-modules. We say that \textit{$\Pi$ holds locally for $A$} if $\Pi_{\frak{a}}$ holds for every $\frak{a} \in\spec(A)$.  
\end{definition}
...
\end{prop}
\begin{proof}
If $N = \left\langle m_1, \ldots, m_t \right\rangle \subset M$, then $\overline{M/N} = \left(0\right)$ since $\overline{M/N} = \overline{M}/\overline{N}$. But then $M \setminus N = 0$ thanks to \cref{Nak}. 
\end{proof}
...
\item In general, we say that a collection of elements $u_1, \ldots, u_k \in B$ are \textit{algebraically independent over $A$} if there is some $f(x_1, \ldots, x_k) \in A\left[x_1, \ldots, x_k\right]$ such that $f\ne 0$ and $f(u_1, \ldots, u_k) =0$ in $B$. We say that $u_1, \ldots, u_k \in B$ are \textit{independent transcendentals over $A$} if they are not algebraically independent over $A$.
...
\begin{definition}
If $L \supset K$ is a field extension, we say that $L$ is a \textit{finite extension} if $L$ is finite dimensional as a vector space over $K$. We call $\left[L: K\right]\coloneqq  \dim_K{L}$ the \textit{degree of the extension}.
\end{definition}
...
\medskip
 ...
 Conversely, if $u\in A$ is simple, then for any $x\in A\setminus (u)$ we have that $\left(x,u\right) = \left(1\right)$ since $x$ and $u$ are coprime. This means that we can find $a,b\in A$ such that $ax + bu =1$. Then $[x]\cdot\left[a\right] = [1]$. Hence $[x]$ is a unit, so that $\faktor{A}{(u)}$ is a field. 
...
\item If $u$ is algebraic, then $\left[K(u) : K\right] = \deg{\min_u}$. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
Suppose that $M \supset L \supset K$ is a chain of finite field extensions. Then $M \supset K$ is also finite with $\left[M:K\right] = \left[M:L\right]\left[L:K\right]$.
\end{theorem}
\begin{proof}
...
\begin{enumerate}[label=(\Alph*)]
\item Suppose that $f$ has all of its roots in $K$. Then $L = K$, and $\left[L:K\right]=1$.
\item Suppose that $f$ has exactly one root in $K$ Then $f(x) = \left(x-\alpha\right)g(x)$ with $\alpha \in K$ and $g(x)$ a quadratic irreducible in $K[x]$. Consider $L = \faktor{K[x]}{(g)}$. Then $\left[L: K\right]=2$, and $g$ has a root in $L$. This implies that $g$ has all of its roots in $L$. Hence $L$ is the splitting field for $f$. 
...
\begin{enumerate}
\item Suppose that $g$ has two roots in $K_1$. Then $L= K_1$, so that $\left[L:K\right] =3$.
\item Suppose that $f$ is irreducible in $K_1$. Then $L= K_2 = \faktor{K_1[x]}{(g)}$, so that $\left[L:K\right] =6$.
\end{enumerate}
\end{enumerate}
We conclude that if $L$ is the splitting field for $f$, then $\left[L:K\right] \in \left\{1, 2, 3, 5\right\}$.

\medskip


How can we compute $\left[L:K\right]$ from the coefficients of $f$?
...
\begin{prop}
Suppose that $f$ has no roots in $K$. Then $\left[L:K\right] = 3 \iff \disc{f} \in K^2$.
\end{prop}
\begin{proof}
We know that $f$ is irreducible over $K$. Hence $K_1 = \faktor{K[x]}{(f)}$ is an extension of degree $3$ in which $f$ has a root $\alpha_1$. Note that $\disc(f) \notin K^2 \iff \disc(f) \notin K_1^2$. The ($\Longleftarrow$) direction is obvious. For the reverse direction, suppose, towards a contradiction, that $\disc(f) \notin K^2$ but $\disc(f) \in K_1^2$. This means that $\left[K\left[\sqrt{\disc(f)}\right] :K\right] = 2$ and $K\subset K\left[\sqrt{\disc(f)}\right] \subset K_1$. Thus, $3 = \left[K_1 : K\right] = \left[K\left[\sqrt{\disc(f)}\right] : K\right] \cdot \left[K_1 : K\left[\sqrt{\disc(f)}\right]\right] = 2 \cdot 1$, a contradiction. 
...
Now consider $R_f\coloneqq  \left\{c \in F \mid f(c) = 0\right\}$. Note that $\F_p \subset R_f \subset F$ and that  $R_f = \left\{c\in F \mid \varphi^n(c) = c\right\}$ where $\varphi$ denotes the Frobenius map. But since $\varphi$ is a field automorphism of $F$, so is $\varphi^n$. Hence the fixed points of $\varphi^n$ form a subfield. This means that $R_f$ is a subfield, hence a splitting field for $f$. Thus, $R_f \cong F$. 
\end{proof}
...
Define the \textit{M\"{o}bius function $\mu : \Z_{>0} \to \left\{{-1}, 0, 1\right\}$} by 
\[
...
\begin{remark}
\Cref{mob} completely characterizes the M\"{o}bius function.
\end{remark}
...
\item  If $L^G = K$ and $K \subset P \subset Q \subset L$ is a chain of field extensions, then every homomorphism $\varphi : P \to L$ over $K$ extends to a homomorphism $Q \to L$ in exactly $\left[Q:P\right]$ many ways. 
\end{enumerate}
\end{theorem}
\begin{proof} $ $
\begin{enumerate}[label=(\alph*)]
\item For the ($\Longleftarrow$) direction, note that if $G \leq \aut(L)$, then tautologically $G \leq \gal(L/L^G)$. Hence $\left\lvert{G}\right\rvert \leq \left\lvert{\gal(L/L^G)}\right\rvert = \left[L: L^G\right]$. If $G \leq \gal(L/K)$, then $L \subset L^G \subset L$, so that $\left[L:L^G\right]\leq \left[L:K\right] =n$. This means that $\left\lvert{G}\right\rvert\leq n$.
...
Apply part (b) to $P = K$ and $Q=L$. In this case, (b) implies that if $L^G=K$, then $\left\lvert{\gal(L/K)}\right\rvert = \left[L:K\right]=n$. Thus, we must show that $G = \gal(L/K)$.
...
\begin{definition}
A finite field extension $L \supset K$ is a \textit{Galois extension} if $\left\lvert{\gal(L/K)}\right\rvert = \left[L:K\right]$.
\end{definition}
...
\begin{exmp} $ $
\begin{enumerate}
\item Let $K$ be a field and let $f(x) \in K[x]$ be irreducible of degree $2$. Let $L$ denote the splitting field for $f(x)$. Then $K\left[\sqrt{D}\right]$ where $D = \disc(f) \in K$. In this case, $\left[L:K\right] = 2$, and $\gal(L/K) \subset S_2$ since $D \ne 0$. Thus, $f$ must have distinct roots in $L$. Note that $\gal(L/K) \ne \left\{\id\right\}$, since these roots are not in $K$. This shows that $\gal(L/K) = \left\langle \sigma \right\rangle = S_2$ where $\sigma : L \to L$ is given by $a + b \sqrt{D} \mapsto a-b \sqrt{D}$.
...
\item Suppose that $\Char{K} \notin \left\{2,3\right\}$. Let $f(x) \in K[x]$ be irreducible and monic of degree $3$. Let $D \in K$ denote the discriminant of $f$. Let $L \supset K$ be the splitting field for $f$, so that $L \supset K$ is Galois. Then $$\left\lvert{\gal(L/K)}\right\rvert = \begin{cases} 6 & D \notin K^2  \\ 3 & D \in K^2    \end{cases}.$$ But $\gal(L/K) \subset S_3$. This shows that $$\gal(L/K) = \begin{cases} S_3 & D \notin K^2  \\ A_3 & D \in K^2    \end{cases}.$$
\end{enumerate}
\end{exmp}
...
\begin{enumerate}[label=(\alph*)]
\item $\left[L:P\right] = \left\lvert{\gal(L/P)}\right\rvert$ for any $K \subset P \subset L$. 
\item $\left[L: L^G\right] = \left\lvert{G}\right\rvert$ for any $G \leq \gal(L/K)$. 
\end{enumerate}
...
We compute
\begin{align*}
\left\lvert{\gal(L/P)}\right\rvert & = \left[L:P\right]
\\ \left[P:K\right] & = \frac{\left[L:K\right]}{\left[L:P\right]} = \frac{\left\lvert{\gal(L/K)}\right\rvert}{\left\lvert{\gal(L/K)}\right\rvert}
\\ \left[P:K\right] & = \left[\gal(L/K) : \gal(L/P)\right].
\end{align*}
...
\begin{exmp}
Let $K$ be a field with $\Char{K} \notin \left\{2,3\right\}$. Let $f$ be an irreducible, monic, cubic polynomial over $K$. Let $L$ be the splitting field for $f$. Let $D\coloneqq  \disc{f} \in K \setminus K^2$. Then $\gal(L/K) = S_3$. We get
\begin{align*} L & \supset L^{A_3} \supset K  \\   \gal(L/K)  & \unrhd A_3 \unrhd \left\{e\right\}  . \end{align*} It follows that $L^{A_3}\supset K$ is Galois with $\gal(L^{A_3}/K) \cong \gal(L/K)/A_3 \cong C_2$. In fact, $L^{A_3} \cong K\left[\sqrt{D}\right]$.
\end{exmp}
...
Let $K$ be a field with $\Char{K} \ne 2$ and $f(x) \in K[x]$ be irreducible. Then $f$ is solvable in quadratic radicals if and only if $\left[L:K\right] =2^n$ for some $n$ where $L$ denotes the splitting field for $f$.
\end{theorem}
\begin{proof} $ $

($\Longrightarrow$) 

We have that $L \supset K$ is a tower of quadratic extensions. Hence  $\left[L:K\right] = 2^n$ for some $n$. 
...
We have that $\left[L:K\right] = 2^n$ for some $n\geq 0$ and $\deg{f} = \left[K(\alpha): K\right] \mid \left[L: K\right]$ where $\alpha$ is a root of $f(x)$. Thus, $\left[K(\alpha): K\right]$ equals a power of $2$, so that $f$ is separable. This shows that $L \supset K$ is Galois and thus that $G\coloneqq  \gal(L/K)$ has order $2^n$. It follows that there is some normal series $$ G =G^0 \unrhd G^1  \unrhd \cdots \unrhd G^s = \left\{e\right\}  $$ such that $\faktor{G^{i}}{G^{i+1}} \cong \Z/2$. This induces a tower of field  extensions $$ K= L^{G^0} \subset L^{G_1} \subset \cdots \subset L^{G_s} = L   $$ such that $[L^{G^{i+1}}: L^{G_i}] =2$.
\end{proof}
...
Let $a\in \Z$. Define the \textit{shift functor} $${-}\left[a\right] : \mathbf{CoCh}(\mathbf{Ab}) \to \mathbf{CoCh}(\mathbf{Ab})$$ as follows. Let $\left(M^{\bullet}, d^{\bullet}_{M}\right)$ be a complex.  Form the pair $\left(M^{\bullet}\left[a\right], d^{\bullet}_{M\left[a\right]}\right)$ where $\left(M^{\bullet}\left[a\right]\right)^n \equiv M^{a+n}$ and $\left(d_{M\left[a\right]}\right)^n \equiv ({-1})^a d^{a+n}_M$. If $f^{\bullet}$ is a chain map, then let $\left(f^{\bullet}\left[a\right]\right)^n = f^{a+n}$.
...
For the reverse inclusion, let $\left[b\right] \in \ker{H^n(g)} \subset H^n(B)$. Then $g(b) \in C^n$ must be a coboundary, so that there is some $c\in C^{n-1}$ such that $g(b) = d_C{c}$. Choose a lift $b_1 \in B^{n-1}$ of $c$, meaning that $g(b_1) = c$. Then $b -  d_B{b_1} \in Z^n(B)$, and $\left[b\right] = \left[b - d_B{b_1}\right]$. But $$g(b - d_B{b_1}) = g(b) - g(d_B{b_1}) = g(b) - d_C{g(b_1)} = g(b) - d_C{c} =0.$$ Hence $b - d_B{b_1} \in \ker{g} \subset B^n$. This implies that there exists a unique $a \in A^n$ such that $b- d_B{b_1} = f(a)$. Also, $$   f(d_A{a}) = d_B(f(a)) = d_B(b-d_B{b_1}) = 0   .$$ Since $f$ is injective, we see that $d_A{a} =0$, i.e., $a \in Z^n(A)$. Thus, $H^n(f)(\left[a\right]) = \left[f(a)\right] = \left[b-d_B{b_1}\right] = \left[b\right]$. This proves that $\left[b\right] \in \im{H^n(f)}$.

\medskip


\underline{Exactness at $H^n(C)$:} Let $\left[b\right] \in H^n(B)$. Note that $\delta^n(H^n(g)(\left[b\right])) = \left[a\right]$ where $a\in A^{n+1}$ denotes the unique element such that $f(a) = d_B{b} $. Since $d_B{b} =0$ and $f$ is injective, it follows that $a=0$. Hence $\im{H^n(g)}\subset \ker{\delta^n}$.  

Conversely, let $\left[c\right] \in \ker{\delta^n}$. Choose $b\in B^n$ such that $g(b) =c$ and then the unique $a\in A^{n+1}$ such that $f(a) = d_B{b}$. Thus, $\delta^n(\left[c\right]) =\left[a\right] =0$, so that $a\in B^{n+1}(A)$, i.e., $d_A{a_1} =a$ for some $a_1 \in A^n$. Note that $g(b-f(a_1))= g(b) -g(f(a_1)) = c -0 =c$. Further, 
\begin{align*}
...
\\ & =0.
\end{align*} 
This shows that $b-f(a_1)$ is a cocycle. Thus, $H^n(g)(\left[b-f(a_1)\right]) = \left[g(b- f(a_1))\right] = \left[c\right]$, so that $\left[c\right] \in \im{H^n(g)}$. 

\medskip


\underline{Exactness at $H^{n+1}(A)$:} Let $\left[c\right] \in H^n(C)$ and find $\left[a\right] = \delta^n(\left[c\right])$, where
\[
\begin{tikzcd}
...
\end{tikzcd}
.\] 
Then $H^{n+1}(f)(\left[a\right]) = \left[f(a)\right] = \left[d_B{b}\right] =0$. It follows that $\im{\delta^n} \subset \ker{H^{n+1}(f)}$.

Conversely, let $\left[a\right] \in  \ker{H^{n+1}(f)}$, so that $H^{n+1}(f)(\left[a\right]) = \left[f(a)\right]= 0$. This means that $f(a) = d_B{b}$ for some $b \in B^n$. Then $\delta^n(\left[g(b)\right]) = \left[a\right]$. This shows that $\im{\delta^n} \supset \ker{H^{n+1}(f)}$.
\end{proof}
\pagebreak
...
\begin{definition} $ $
\begin{enumerate}
\item A category $\c$ is \textit{enhanced over $\mathbf{Ab}$} if $\Hom_{\c}(a, b)$ is an abelian group for any $a, b \in \ob{\c}$ and the function $$\Hom_{\c}(y, z) \times \Hom_{\c}(x,y) \overset{\circ}{\longrightarrow} \Hom_{\c}(x,z)$$ is bilinear for any $x,y,z\in \ob{\c}$.
...
\item Any additive category has finite coproducts, and these are equal to finite products.
\end{enumerate}
\end{exercise}
...
If $\c$ is strongly pre-triangulated, then $\ho^{\bullet}(\c)$ and $\ho(\c)$ contain the same information. Indeed, $\ho(\c)$ is precisely the degree zero piece of $\ho^{\bullet}(\c)$. Conversely, $if x,y \in \ob{\c}$, then $$\Hom_{\ho^{\bullet}(\c)}(x,y) = \bigoplus_{a\in \Z}\Hom^a_{\ho^{\bullet}(\c)}(x,y)$$ where $\Hom_{\ho^{\bullet}(\c)}^a(x,y)  =  \Hom_{\ho(\c)}(x, y\left[a\right]) = \Hom_{\ho(\c)}(x,y)\left[a\right]$. 
...
\begin{exercise} Prove the following assertions.
\begin{enumerate}
\item Both $x/ \q$ and $\left(\q/x\right)^{\op}$ are directed categories. 
...
\begin{definition}
If $\a$ is an abelian category, then the \textit{derived category of $\a$} is the triangulated category $$ \D(\a) \equiv \faktor{\mathcal{C}(\a)}{\mathcal{C}(\a)^{\acyc}}  , $$ where $\mathcal{C}(\a)^{\acyc}$ is the full subcategory of $\mathcal{C}(\a)$ on those complexes with zero cohomology. 
\end{definition}


To do computations in $\D(\a)$, we must understand when it can be embedded in $\mathcal{C}(\a)$ so that $\left(\mathcal{C}(\a)^{\acyc}, \D(\a)\right)$ is an adjoint pair. This requires $\left(\mathcal{C}(\a)^{\acyc}\right)^{\perp}$  to be large. 
