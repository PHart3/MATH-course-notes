...
\begin{exercise}[Freudenthal suspension theorem]
Let $i \in \Z_{\geq 1}$ and suppose that the space $X$ satisfies $\pi_n(X) =0$ for each $0\leq n \leq i-1$. Show that $\pi_i(X) \cong \pi_{i+1}(S{X})$ through around dimension $2i-3$ (figure this out exactly). (For extra credit, find a spectral sequence proof of the relative Hurewicz theorem.)
\end{exercise}
\begin{proof}
...
A \arrow[r]                        & C          
\end{tikzcd}
\] is a pushout diagram. Also, suppose that $\pi_n(B, X)=0$ and $\pi_m(A, X)=0$ for each $0\leq n \leq k_1$ and each $0\leq m \leq k_2$. The map $\left(A, X\right) \to \left(C, B\right)$ of pairs itself induces an isomorphism $\pi_n(A, X) \overset{\cong}{\longrightarrow} \pi_n(C, B)$ for each $0 \leq n \leq k_1 + k_2 -1$.
\end{theorem}
...
Recall the space $B_{\Un(n)} = B_{\GL(n, \C)}$ of $n$-planes in $\C^{\infty}$ as well as the space $B_{\Or(n)} = B_{\GL(n, \R)}$ of $n$-planes in $\R^{\infty}$. We want to describe the graded rings 
$H^{\ast}(B_{\Un(n)}; \Z)$ and  
...
Consider the differential $d_2 : \underbrace{H^{2n-1}(S^{2n-1})}_{\Z} \to H^{2n}(B{\Un(n)})$ and pick a generator (i.e., an orientation) $g_n$ of $H^{2n-1}(S^{2n-1})$. Let
\[
c_n= d_{2}(g_n).
\]
