...
$H^n(X) \cong \frac{\ker{\delta^{n+1}}}{\im{\delta^n}}$.
\end{theorem}
...
Let $X$ be a cell complex. Recall the \textit{cup product} operation $H^i(X) \times H^j(X) \overset{\smile }{\longrightarrow} H^{i+j}(X)$ on cohomology, which is both bilinear and anti-commutative in the sense that
\[
x \smile y = ({-1})^{i+j}y \smile x.
\]
...
\] and, in light of the equation $\partial(D^i \times D^j) = \left(\partial{D^i} \times D^j\right) \cup \left(D^i \times \partial{D^j}\right)$, that
\[
