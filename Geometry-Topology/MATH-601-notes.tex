...
\begin{raggedright}
{\underline{Basic geometric notions}} 
\end{raggedright}
...
\begin{corollary}{\textbf{(Path lifting property)}}
\begin{enumerate}
...
\begin{corollary}{\textbf{(Brouwer fixed point theorem in dimension 2)}}
...
\begin{lemma}{\textbf{(Homotopy lifting property)}}
Let $F : Z \times I \to X$ be any map and $p: Y \to X$ be a covering projection. Suppose that there is some $\widetilde{F}_0 : Z \times \{0\} \to Y$ such that $p \circ \widetilde{F}_0 = F_0$. Then there exists a unique map $\widetilde{F} : Z \times I \to Y$ such that $p \circ \widetilde{F} = F$ and $\widetilde{F} \restriction_{Z\times \{0\}} = \widetilde{F}_0$.
\end{lemma}
\begin{proof}
Let $z_0\in Z$.
...
\begin{theorem}{\textbf{(van Kampen)}}
...
\begin{note}{\textbf{(Wirtinger presentation)}} 
...
\begin{lemma}{\textbf{(Lifting criterion)}}
...
\begin{prop}{\textbf{(Unique lifting property)}}
...
\begin{remark}{\textbf{(Singular simplex)}}
...
\begin{note}
Our proof of the last theorem shows that the functor $C_{\ast}(-) : \mathbf{Top} \to \mathbf{Ch}$ preserves coproducts. 
\end{note}
...
\begin{definition}{\textbf{(Reduced homology)}}
...
\begin{definition}{\textbf{(Relative homology)}}
...
\begin{lemma} Let $X$ be a path connected space and $A\subset X$.
\begin{enumerate}
\item $H_0(X) \cong \Z$.
...
\begin{note} By Lemma 21, the bottom row
\[
\begin{tikzcd}
H_0(A) \arrow[r]         & H_0(X) \arrow[r]     & {H_0(X,A)} \arrow[r]     & 0
\end{tikzcd}
...
\end{corollary}
\begin{proof}
Note that $\bigvee_{\alpha} X_{\alpha} = \faktor{\coprod_{\alpha}X_{\alpha}}{\coprod_{\alpha} \{x_{\alpha}\}}$. Now apply Corollary 12 together with the fact that the functor $H_{\ast}({-}, {-}): \mathbf{Top}\times \mathbf{Top} \to \mathbf{Ab}$ preserves coproducts. 
\end{proof}
...
\begin{lemma}{\textbf{(Lebesgue covering)}}
...
\begin{proof}{\textbf{(Excision theorem)}}
We prove the equivalent statement expressed in Lemma 25. Write $X = \mathring{A}\cup \mathring{B}$ with $A,B\subset X$. Let $\U =\{A, B\}$. Note that $C_{\ast}(A)$ is a subcomplex of $C_{\ast}^{\U}(X)$. The diagram
\[
\begin{tikzcd}
...
\begin{corollary}{\textbf{(Mayer-Vietoris (MV) sequence)}}
...
\begin{theorem}{\textbf{(Brouwer's invariance of domain)}}
Let $U\subset \R^n$ be open and $V\subset \R^m$ be open. If $U \cong V$, then $n=m$.
\end{theorem}
\begin{proof}
Let $x\in U$ and consider $H_{\ast}(U \setminus \{x\})$. By excision, we get $H_{\ast}(U, U \setminus \{x\}) \cong H_{\ast}(\R^n, \R^n \setminus \{x\})$. By Corollary 10, we also get a LES
\[
\begin{tikzcd}
...
\end{tikzcd}
\] where the two upper isomorphisms come from excision and the two lower isomorphisms come from the LES of Corollary 10. Therefore, the homomorphism $f_{\ast} : H_n(U_i, U_i \setminus \{x_i\}) \to H_n(V, V\setminus \{x\})$ can be viewed as a homomorphism $f_{\ast} : \Z \to \Z$, so that $f_{\ast}(g) = \alpha g$ for some $\alpha \in \Z$. We call $\alpha$ the \textit{local degree of $f$ at $x$}, written as $\deg_{x_i}{f}$.
\end{note}
...
\begin{definition}{\textbf{(Cellular homology)}}
...
\begin{prop}{\textbf{(Cellular boundary formula)}}
...
\item Let $X$ be a space whose singular chain complex is finite. The \textit{Euler characteristic $\chi(X)$ of $X$} is $ \sum_n({-}1)^n\rnk{H_n(X)}$, which is a finite sum by Corollary 19.
\end{enumerate}
\end{definition}
...
\begin{definition}{\textbf{(Homology with coefficients)}}
...
\begin{theorem}{\textbf{(Universal coefficient theorem (for cohomology))}}
...
\begin{theorem}{\textbf{(K\"unneth (special case))}}
...
\begin{theorem}{\textbf{(Poincar\'e duality (PD))}}
...
\begin{prop}{\textbf{(Projection formula)}}
...
\begin{remark}{\textbf{(De Rham homology)}}
...
\begin{theorem}{\textbf{(Lefschetz duality)}}
...
\begin{remark}{\textbf{(\v{C}ech cohomology)}}
...
\begin{definition}{\textbf{(Sheaf cohomology)}}
...
\begin{exmp}{\textbf{(Godement's construction)}}
...
\begin{prop}{\textbf{(De Rham theorem for local systems)}}
