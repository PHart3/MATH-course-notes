...
\begin{definition}
If $K$ is a simplicial complex, then define $\lvert{K}\rvert$ as the set $$\{\alpha : K \to I \mid \sum_{v\in K} \alpha(v)=1 \land \{v \mid \alpha(v) \ne 0\} \in \Sigma_K \}.$$ Let $\lvert{K}\rvert_d$ denote this set endowed with the topology induced by the metric given by $$d(\alpha, \beta) = \sqrt{\sum_{v\in K} |\alpha(v) -\beta(v)|^2} .$$
\end{definition}
...
\begin{note} $ $
\begin{enumerate}
\item When $\sigma \in \Sigma_K$ is a $q$-simplex, there exists a natural map $|\sigma^q|_d : \Delta^q \to \lvert{K}\rvert_d$  given by mapping $x$ to the function $\alpha$ where $\alpha(\sigma) =x$ and $\alpha(v) = 0$ for any $v\notin \sigma$.  
\item Define a new topology on $\lvert{K}\rvert$ where $A\subset \lvert{K}\rvert$ is closed (resp. open) if $(|\sigma|_d)^{-1}(A)$ is closed (resp. open) in $\Delta^{|\sigma| -1}$ for each $\sigma \in \Sigma_K$. From now on, $\lvert{K}\rvert$ is assumed to have this topology. 
\item Given a simplicial map $f: K \to L$, define the map $|f| : \lvert{K}\rvert \to \lvert{L}\rvert$ by $\alpha \mapsto \left(w \mapsto \sum_{v\in f^{-1}(w)} \alpha(v)\right)$. Thus, we get a functor $\lvert{\cdot}\rvert : \Delta \to \mathbf{Top}$.
\end{enumerate}
\end{note}
...
\begin{definition} $ $
\begin{enumerate}
\item Given a chain complex $\left(A, \partial\right)$, define its \textit{$i$-th homology group} as $$ H_i(A, \partial) = \faktor{Z(A_i)}{B(A_i)} $$ where $\underbrace{Z_i \coloneqq  \ker{\partial_{i}}}_{\textit{cycles}}$ and $\underbrace{B_i \coloneqq  \im{\partial_{i+1}}}_{\textit{boundaries}}$. 
...
\item We get a functor $C_{\bullet}: \Delta \to \ch$ given by $\left(K, \Sigma\right) \mapsto \left(C_q(K, \Sigma), \partial_{q}\right)_{q\geq 0}$. As a result, we find ourselves with the following diagram of functors.
\[
\begin{tikzcd}
...
\begin{note}
Suppose that $F : X \times I \to Y$ is a homotopy from $f$ to $g$. Then we want to show that $H_q(f) =H_q(g)$ for each $q$. Although $\Delta^q \times I $ is not a simplex, we can write $\Delta^q \times \{0\} = \left[v_0, \ldots, v_q\right]$ and $\Delta^q \times  \{1\} = [v_0', \ldots, v_q']$ and consider the following decomposition of  $\Delta^q \times I$ into simplices. 
\begin{gather*}
\left[v_0, v_1,  \ldots, v_q\right]
\\ \left[v_0, \ldots,  v_q, v_q'\right]
\\ \left[v_0, \ldots,  v_{q-1}  , v_{q-1}', v_q'\right]
\\  \vdots
\\ \left[v_0', v_1',  \ldots, v_q'\right]
\end{gather*}
\end{note}
...
\begin{definition}
Let $(K, \Sigma)$ be a simplicial complex and $L \subset K$ a subcomplex. Then  $\faktor{C_{\ast}(K)}{C_{\ast}(L)} = C_{\ast}(K,L).$ Recall that $$\lvert{K}\rvert=\{f: K \to I \mid \{v\in K \mid f(v) \ne 0\} \in \Sigma, \ \sum_{v\in K}f(v) =1\} .$$ Define $\varphi : C_k(K) \to C_k(\lvert{K}\rvert)$ by $\left[v_0, v_1, \ldots, v_k\right] \mapsto \left(\sigma_{[v_0, \ldots, v_k]} : \Delta^k \to \lvert{K}\rvert\right)$ where $$\sigma_{[v_0, \ldots, v_k]}(t_0, \ldots, t_k)(v \in K) = \begin{cases} t_i & v=v_i \\ 0 & v \ne v_i \end{cases},$$ extended by linearity. This induces a map $\varphi : C_{\ast}(K) \to C_{\ast}(\lvert{K}\rvert)$.
\end{definition}
...
\begin{prop}\label{p1}
A map $\psi : C \to \lvert{K}\rvert$ with $C$ a compact space has $\im{\psi} \subset |K^i|$ for some $i$.
\end{prop}

\begin{corollary}
$H_{\ast}(\lvert{K}\rvert) = \underbrace{\varinjlim H_{\ast}(|K^i|)}_{\text{direct limit}}$.
\end{corollary}
\begin{proof}
Any cycle $c\in C_i(\lvert{K}\rvert)$ is finite linear combination of singular simplices. \Cref{p1} implies that there exists $j\geq 0$ small enough so that $c \in  C_i(K^j|)$ and $[c]\ne 0 \in H_i(|K^j|)$.
\end{proof}
...
\begin{definition}
 A \textit{presheaf on $X$} is a functor $F: \Op(X)^{\op} \to \mathbf{Ab}$ such that $F(\emptyset) =0$. If $V\subset X$ is open, then any $s\in F(V)$ is called a \textit{section of $U$}. If $i : U \hookrightarrow V$, then $F(i): F(V) \to F(U)$ is called a \textit{restriction morphism} and is denoted by $r_{UV}$. Let $r_{UV}(s)\coloneqq  s \restriction_U$. 
\end{definition}
