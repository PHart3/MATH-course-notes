...
\\ &=  i\partial{u}.
\end{align*}
Therefore, $\partial{b'} =i(a -\partial{u})$. Since $[a] = \left[a-\partial{u}\right] $ in $H_{k-1}(A)$, we see that $a$ is independent of our choice of $b$. From now on, we shall denote such an $a$ by $a_c$. It is clear that $\partial '$ is also independent of our choice of $c\in \ker{\partial}$ and thus is well-defined. Finally, it is straightforward to check that it is a homomorphism.
\end{proof}
...
$$ j( g -y) =j(g) - j(y) =f - \underbrace{j( i(\tilde{f}))}_{ji =0} = f.$$ Therefore, $\partial(g - y) = i(a_c) - i(a_c) =0$, so that $g- y \in \ker \partial = Z(B_k)$. Since $\left[j  (g-y)\right] = [f]$, it follows that $[f] \in \im{j  (\bullet)}$, i.e., $ \im{j  (\bullet)} \supset \ker{\partial'}$.
...
\end{corollary}
\begin{proof}
Any cycle $c\in C_i(\left\lvert{K}\right\rvert)$ is finite linear combination of singular simplices. \Cref{p1} implies that there exists $j\geq 0$ small enough so that $c \in  C_i(\left\lvert{K^j}\right\rvert)$ and $[c]\ne 0 \in H_i(\left\lvert{K^j}\right\rvert)$.
\end{proof}
...
\begin{lemma}
$\partial(\sigma \frown\varphi) = \left({-1}\right)^l(\partial{\sigma} \frown \varphi - \sigma \frown \delta{\varphi})$.
\end{lemma}
\begin{proof}
We compute
\begin{align*}
...
\end{align*}
It's easy to check that $\left({-1}\right)^l \partial(\sigma \frown \varphi) = \partial{\sigma} \frown \varphi - \sigma \frown \delta{\varphi}.$
\end{proof}
...
\begin{definition}
Let $X$ be a closed $R$-orientable $n$-manifold. Consider the bilinear form $$B: H^k(X; R) \times H^{n-k}(X;R) \to R$$ given by $\left(f,g\right) \mapsto \left(f\smile g\right)[X]$. 
...
\end{theorem}
\begin{proof}
We shall suppress our notation for the coefficient ring $R$. Let $C$ denote the directed set of compact sets $K$ contained in a complement of collar neighbohood of $\partial{M}$. Observe that
\begin{align*}
