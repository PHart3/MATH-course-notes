...
p^{-1}(U) \arrow[rd, "p"'] \arrow[r, "\cong"] & U \times S \arrow[d, "\pi_1"] \\
 & U
\end{tikzcd}
\] commutes for some discrete set $S$.
\end{definition}
...
\item  If every $S$ has $\lvert{S}\rvert = n$ for some $n\in \N$, then $p$ is called an \textit{$n$-fold cover of $X$}.
\end{enumerate}
\end{term}
...
\begin{corollary}
Let $p: \left(Y, y_0\right) \to \left(X, x_0\right)$  be a covering map. Then $p_{\ast} : \pi_1\left(Y, y_0\right) \to \pi_1\left(X, x_0\right)$ is injective. 
\end{corollary}
\begin{proof}
...
\begin{lemma}
If $p: \left(Y, y_0\right) \to \left(X, x_0\right)$ is a covering map, then there is an isomorphism of sets $$p^{-1}(x_0) \overset{\cong}{\longrightarrow} \faktor{\pi_1\left(X, x_0\right)}{p_{\ast}\pi_1\left(Y, y_0\right)}.   $$
\end{lemma}
\begin{proof}
...
Let $p: \left(Y, y_0\right) \to \left(X, x_0\right)$ be a covering map and $f: (Z, z_0) \to \left(X, x_0\right)$ a map with $Z$ path connected and locally path connected.  Then there exists some map $\tilde{f}$ fitting into the commutative diagram \[
\begin{tikzcd}
 & Y \arrow[d, "p"] \\
...
If $X$ is a swell space, then we call such a cover $p: \left(\widetilde{X}, \tilde{x}_0\right) \to \left(X, x_0\right)$ the \textit{universal cover of $X$}. It is universal in that for any path connected covering space $p': \left(Y, y_0\right) \to \left(X, x_0\right)$, there exists a unique covering map $p'': \left(\widetilde{X}, \tilde{x}_0\right) \to \left(Y, y_0\right)$ such that $p'' p' = p$. 
...
\begin{lemma}
Let $X$ be path connected and locally path connected. Suppose that $p_1:(Y_1, y_1) \to \left(X, x_0\right)$ and $p_2 : (Y_2, y_2) \to \left(X, x_0\right)$ are two (path connected) covering maps. Then there exists an isomorphism $\varphi : (Y_1, y_1) \to (Y_2, y_2)$ if and only if  $$p_{1\ast} \pi_1(Y_1, y_1) = p_{2 \ast} \pi_1(Y_2, y_2).$$
\end{lemma}
\begin{proof} $ $
\smallskip
