...
\end{lemma}
\begin{proof}
Let $\left(U, \varphi\right)$ be a chart centered at $p$. Write $v = v_i \frac{\partial}{\partial{x_i}}\bigr\rvert_{p}$. For any $\epsilon >0$ small, define $\gamma: \left({-\epsilon} , \epsilon\right) \to U$ by $\gamma(t) = \varphi^{-1}(tv_1, \ldots, tv_n)$. \Cref{rem} implies that $\gamma'(0) = v$.
\end{proof}
...
$X(f \circ F) = \left(Yf\right) \circ F$ if and only if $X$ and $Y$ are $F$-related.
\end{note}
...
We say that an element of $$V_{\ell}^k\coloneqq  \underbrace{V^{\ast} \otimes \cdots \otimes V^{\ast}}_{k \text{ copies}} \otimes \underbrace{V \otimes \cdots \otimes V}_{\ell \text{ copies}}$$ is a \textit{$\left(k, \ell\right)$-tensor}. 
\end{definition}
...
\begin{term} $ $
\begin{enumerate}
\item A $\left(k, 0\right)$-tensor is called \textit{covariant}. 
\item A $\left(0, \ell\right)$-tensor is called \textit{contravariant}.
\end{enumerate}
\end{term}

\smallskip

Let $M$ be a smooth manifold. Define the \textit{$\left(k, \ell\right)$-tensor bundle} as $$T _{\ell}^k M \equiv \coprod_{p\in M}\left(T_p\right)_{\ell}^k M.$$
...
Let us examine the form of a generic $\left(k,0\right)$-tensor. Suppose that $\left(x^i\right)$ and $\left(y^i\right)$ are two local coordinate systems around a point $p\in M$. Then 
\begin{align*}
...
\begin{definition}
A \textit{$\left(k, \ell\right)$-tensor field} is a (smooth) section of $T_{\ell}^kM$. 
\end{definition}
...
Let $\left(U, x^i\right)$ be local coordinates for $M$. Then any $A \in \T_k^{\ell}(M)$ can be written in $U$ as $$A\bigr\rvert_p = A_{i_1 \ldots i_k}^{j_1\ldots j_{\ell}} dx^{i_1}\bigr\rvert_p \otimes \cdots \otimes dx^{i_k}\bigr\rvert_p \otimes \frac{\partial}{\partial{x^{j_1}}}\bigr\rvert_p \otimes \cdots \otimes \frac{\partial}{\partial{x^{j_{\ell}}}}\bigr\rvert_p,$$ summed over $n^kn^{\ell}$ many tensors.
...
\begin{align*}
\sigma(X, w) & = \delta_j^i dx^j \otimes \frac{\partial}{\partial{x^i}}\left(X^k \frac{\partial}{\partial{x^k}}, w_{\ell}dx^{\ell}\right) 
\\ & = \delta_j^i dx^j\left(X^k \frac{\partial}{\partial{x^k}}\right) \frac{\partial}{\partial{x^i}} w_{\ell}dx^{\ell} 
...
\item
Let $\sigma$ be a singular $k$-cell on $M$. Let $i=1, \ldots, 2k$ and $\alpha =0,1$. Define the \textit{$\left(i, \alpha\right)$-face of $\sigma$} as the smooth map $\sigma_{(i, \alpha)}$ given by $$\sigma_{(i, \alpha)}(x^1, \ldots, x^k) = \sigma(x^1,\ldots, x^{i-1}, \alpha, x^{i}, \ldots, x^k).$$ Moreover, define the \textit{boundary of $\sigma$} as the $\left(k-1\right)$-chain $$  \partial{\sigma} \equiv \sum_{i=1}^k\left({-1}\right)^{i+1}(\sigma_{(i,1)}-\sigma_{(i,0)}) .$$
\item
If $\sigma\coloneqq  \sum_{i=1}^Na_i\sigma_i$ is a singular $k$-chain, then define the \textit{boundary of $\sigma$} as the $\left(k-1\right)$-chain $$\partial{\sigma} \equiv \sum_{i=1}^N a_i \partial{\sigma_i}   .$$ 
\end{enumerate}
\end{definition}
...
\begin{theorem}
Let $M$ be a smooth $n$-manifold.  Let $\left(X_1, \ldots, X_k\right)$ be a linearly independent $k$-tuple of smooth commuting vector fields defined on an open set $W\subset M$. For any $p\in W$, there is some chart $\left(U, x^i\right)$ around $p$ such that $$X_i = \frac{\partial}{\partial{x^i}}$$ on $U$ for each $i=1, \ldots, k$.
\end{theorem}
\begin{proof}[Proof sketch]
...
\begin{definition}
We say that a distribution $D$ is \textit{involutive} if $\left[X,Y\right]\in D$ whenever $X,Y \in D$. 
\end{definition}
