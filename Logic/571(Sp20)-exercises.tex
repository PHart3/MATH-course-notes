...
M_1 < M_2 < M_3 <\cdots
 ...
 \begin{algorithm}[H]
    ...
    {run $U_{\TM}$ on $\langle M_i, x\rangle$}\;
         \eIf{$U_{\TM}$ rejects}{reject}{run $U_{\TM}$ on $\langle M_i,y\rangle$ for all strings $y$ such that $\left\lvert{y}\right\rvert \leq \left\lvert{x}\right\rvert$\;\eIf{$U_{\TM}$ halts for each such $y$}{accept}{reject}}
