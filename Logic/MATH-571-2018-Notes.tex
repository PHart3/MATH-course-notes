...
\begin{abstract}
These notes, which are far from finished, are based on Scott Weinstein's ``Topics in Logic: Set Theory'' lectures at UPenn along with  Thomas Jech's \textit{Set Theory - The Third Millennium Edition, revised and expanded}. Any mistake in what follows is my own.
\end{abstract}
...
Throughout this section, we will  work tacitly in $\zfc$.
...
\begin{notation}
If two sets $X$ and $Y$ are equipollent, then we will write either $X \sim Y$ or $\lvert{X}\rvert = \lvert{Y}\rvert$.
\end{notation}
...
\end{theorem}
\begin{proof}
It's obvious that $\R$ is not finite. Suppose, towards a contradiction, that $\R$ is countable. Enumerate $\R$ as $$\R = \{x_0, x_1, \ldots, x_n, \ldots\}.$$ Define the sequences $(a_n)_{n\in \N}$ and $(b_n)_{n\in \N}$ recursively as follows. Let $a_0 = x_0$ and $b_0 = x_{k_0}$ where $k_0$ denotes the least $k$ such that $a_0 < x_k$. For each $n\in \N$, let $a_{n+1}= x_{i_n}$ where $i_n$ denotes the least $i$ such that $a_n < x_i < b_n$ and let $b_{n+1} = x_{k_n}$ where $k_n$ denotes the least $k$ such that $a_{n+1} < x_k < b_n$. In terms of our enumeration of $\R$, we have that
\begin{align*}
...
\begin{exmp} $ $
\begin{enumerate}
\item Let $X = \{0\} \cup [1, \frac{3}{2}) \cup (\frac{3}{2}, 2] \cup \{3\}$. Then the isolated points of $X$ are precisely $0$ and $3$. 
\item Let $X = \{0\} \cup \{\frac{1}{n} \mid n \in \Z_{>0}\}$. Then the isolated points of $X$ are precisely $\frac{1}{n}$ for each $n\in \Z_{>0}$.
\end{enumerate}
\end{exmp}
...
\end{theorem}
\begin{proof} 
 By hypothesis, there is some isomorphism $f: P \to \Q$. Extend $f$ to $\tilde{f} : C \to \R$ as follows. For any $c \in C \setminus P$, let $$\tilde{f}(c)  = \sup\{f(p) \mid p\in P \land p < c\}.$$ It is easy to check that $\tilde{f}$ is order-preserving. It remains to show that it is surjective. If $r\in \R \setminus \Q$, then there is some strictly increasing sequence of rationals $(q_n)_{n\in \N}$ that converges to $r$. In particular, $\sup\{q_n \mid n\in \N\}  = r$. Then $\tilde{f}$ sends $\sup\{f^{-1}(q_n) \mid n \in \N\}$ to $r$.
\end{proof}
...
\end{theorem}
\begin{proof}
Set $C$ equal to the set of all Dedekind cuts in $P$. Let $(A_1, B_1)\preceq (A_2, B_2)$ if $A_1\subset A_2$. It's easy to check that $(C, \prec)$ is  a linear ordering. Also, since $P$ is unbounded, so is $C$. If $T\coloneqq \{(A_s, B_s) \mid s\in S\}$ is any nonempty bounded subset of $C$, then $$(\bigcup_{s\in S} A_s, \bigcap_{s\in S} B_s) = \sup(T).$$ Thus, $C$ is complete. 
\\ \\
Now, for each $p\in P$, let 
\begin{align*}
A_p & = \{x\in P \mid x<p\}
\\ B_p & = \{x\in P \mid x\geq p\}.
\end{align*}
Note that each $(A_p, B_p)$ belongs to $C$ and that $$P' \coloneqq \{(A_p, B_p) \mid p\in P\} \cong P.$$ Suppose that $(A_1, B_1) \prec (A_2, B_2)$, so that $A_1 \subsetneq A_2$. Then there is some $x\in A_2 \setminus A_1$. Pick any $x' \in A_2$ such that $x<x'$. Note that $x\in A_{x'} \setminus A_1$ and $x'\in A_2\setminus A_{x'}$. Therefore, $A_1 \subsetneq A_{x'} \subset A_2$. This implies $(A_1, B_1) \prec (A_{x'}, B_{x'}) \prec (A_2, B_2)$. It follows that $P'$ is dense in $C$. 
\end{proof}
...
\begin{enumerate}
\item Take the interval $$C_0 \coloneqq [0,1].$$
\item Remove $(\frac{1}{3}, \frac{2}{3})$ from $I_0$ to get $$C_1\coloneqq [0, \frac{1}{3}] \cup [\frac{2}{3}, 1].$$
\item Remove $(\frac{1}{9}, \frac{2}{9})$  and $(\frac{7}{9}, \frac{8}{9})$ from $I_1$ to get $$C_2 \coloneqq [0, \frac{1}{9}] \cup [\frac{2}{9}, \frac{1}{3}] \cup [\frac{2}{3}, \frac{7}{9}] \cup [\frac{8}{9}, 1]      .$$
...
\end{claim}
\begin{proof}
Let $\epsilon >0$. Choose $n\in \N$ so large that $(\frac{1}{3})^n <\epsilon$. Let $x\in \bm{\mathcal{C}}$. Then $x\in C_n$, so that $x$ belongs to some closed interval of length $\frac{1}{3}^n$. Pick $y \ne x$ such that $y$ is an endpoint of this interval. Then $y\in \bm{\mathcal{C}}$, and $|y-x| < \epsilon$. This shows that any open interval around $x$ contains a point in $\bm{\mathcal{C}}$ other than $x$. Thus, $x$ is not an isolated point.
\end{proof}$ $
...
\\ \\By induction on the length of string, we will define a set of closed intervals $(I_w)_{w\in \{0,1\}^{\ast}}$ such that
\begin{itemize}
\item $\lvert{I_w \cap P}\rvert >2$,
\item $I_{w\sigma}\subset I_w$,
\item $I_{w0} \cap I_{w1} = \emptyset$, and
\item $\lvert{I_s}\rvert < \frac{1}{|w|+1}$.
\end{itemize}
where $\sigma \in \{0,1\}$. For the base case, there exists a closed interval $I_{\epsilon}$ such that $\lvert{I_{\epsilon}}\rvert < 1$ and $\lvert{I_{\epsilon}\cap P}\rvert >2$ since $P$ has no isolated points. In addition, by our previous lemma, we can find disjoint subintervals $I_1$ and $I_0$ each on length $< \frac{1}{2}$ such that $\lvert{I_1 \cap P}\rvert>2$ and $\lvert{I_0 \cap P}\rvert >2$. Moreover, the same lemma automatically completes our induction step. 
...
\end{claim}
\begin{proof}
For each $I_{\bar{f}(k)}$, there is some $x_k \in P \cap I_{\bar{f}(k)}$. Then $\lim_{n\to \infty} |x_n - c_f| = 0$, so that $x_n \to c_f$. Since $P$ is closed and $c_f$ is a limit point of $P$, it follows that $c_f \in P$. 
\end{proof}$ $
...
Hilbert placed the resolution of $\mathsf{CH}$ first on his famous 1900 research agenda for mathematics in the twentieth century. W. Hugh Woodin, a prominent set theorist, has recently argued that $\mathsf{CH}$ is true. It's clear that every nonempty open set is equipollent to $\R$. Moreover, our last corollary shows that any closed set is either finite, equipollent to $\N$, or equipollent to $\R$. Therefore, no closed or open subset of $\R$ is a counterexample to $\mathsf{CH}$. Our next result, however, shows that we \emph{cannot} confirm $\mathsf{CH}$ by showing that every uncountable subset of $\R$ contains a perfect set.
\end{remark}
...
\begin{definition}
Define the \textit{rank hierarchy (of sets)} by ordinal recursion as the following sequence of objects (viewed as formal or primitive objects).\footnote{This is also known as the \textit{cumulative hierarchy}, but we will use this term for a more general concept later.}
\begin{align*}
V_0 & = \emptyset
...
\item \textit{Russell:} Originally, Frege proposed the \textit{axiom schema of unrestricted comprehension}: If $P$ is a first-order property, then there exists a set $Y = \{x \mid P(x)\}$. In 1902, Russell found that unrestricted comprehension is false. Indeed,  it implies that $$S \coloneqq \{x \mid x \notin x\}$$ is a set. But, in this case, $S \in S  \iff S \notin S$, which is false. 
\end{enumerate}
...
Define $X \cup Y = \bigcup \{X, Y\}$ and $\{a,b,c\} = \{a,b\} \cup \{c\}$.
\item \textit{Power set}  ($\mathsf{PowerSet}$):
\[
\forall X \exists Y \forall u(u \in Y \leftrightarrow u \subset X)
\]
where $u \subset X \coloneqq \forall z(z \in U \rightarrow z \in X)$.
...
\[
\forall u ((\exists x (x\in u)) \rightarrow (\exists m \forall y(y\in u \rightarrow \neg(u \in m)))).
\]
\end{enumerate}
\end{definition}
...
\begin{definition}
Consider the \textit{von Neumann universe} or \textit{universal class} $$V \coloneqq \bigcup_{\alpha \in \ord}V_{\alpha},$$ which is the proper class $ \{x \mid V(x)\}$ of all sets  belonging to some stage of the rank hierarchy. 
\end{definition}
...
\end{note}
 ...
 \begin{remark} 
In 1908, Zermelo introduced the first six axioms of $\zf$ (along with the axiom of choice). In  1922, Fraenkel and Skolem together introduced the axiom schema of replacement to ensure that objects like $V_{\omega + \omega}$ counted as a set. Finally, in 1925, von Neumann  introduced the axiom of regularity to enable proofs by induction on so-called well-founded proper classes, such as $\ord$.
\end{remark}

\begin{remark}
In $\zf$, we avoid each of the three paradoxes from Remark 3 by replacing unrestricted comprehension with $\mathsf{Sep}$. In fact, they serve as proofs that the following classes are proper. 
\begin{enumerate}
\item The class of all ordinals (see Corollary 7 below).
...
\begin{definition}
For any sets $x$ and $y$, the \textit{ordered pair $(x,y)$} is the set $\{\{x\}, \{x,y\}\}$, which exists by $\mathsf{Pair}$.
\end{definition}
...
\item $\N$ ordered as $\{0, 2, 4, 6, \ldots, 1, 3, 5, 7, \ldots\}$ is a woset.
\end{enumerate}
\end{exmp}
...
\begin{proof} $ $
\begin{enumerate}[label=(\arabic*)]
\item Let $f: (W, <) \to (W,<)$ be an automorphism.  Let $x\in W$. By our previous lemma, we see that $f(x)\geq x$ and $x = f^{-1}(f(x))\geq f(x)$. Hence $f(x) =x$.
...
\end{theorem}
\begin{proof}
First, note that our previous lemma implies that no two of (a), (b), and (c) can occur simultaneously. We must show that at least one occurs. Consider the set $$ f \coloneqq \{(w,y) \in W\times Y \mid W_w \cong Y_y\}  .$$ If $Y_y \cong Y_{y'}$, then $y = y'$ by our previous lemma. Hence $f$ is a partial function. Moreover,  if $w,w'\in \dom{f}$ and $w<w'$ with $h : W_{w'} \overset{\cong}{\longrightarrow} Y_{f(w')}$, then $W_w \cong Y_{h(w)}$, so that $f(w) = h(w)<' f(w')$. It follows that $f$ is order-preserving. 
...
\begin{notation}
We will use the symbols $\in$ and $<$ interchangeably for the ordering of ordinals. 
\end{notation}
...
\end{lemma}
\begin{proof}
It is clear that $\in$ is set-like. Let $S \subset \ord$ be a nonempty subset. We must show that $S$ has a minimal element. Note that $\bigcap{S} \subset \alpha$ for every $\alpha \in S$. Therefore, $(\bigcap{S}, \in)$ is a  well-ordering as the restriction of a well-ordering. Now, suppose that $\beta \in \bigcap{S}$. Then $\beta \in \alpha$ for every $\alpha \in S$. By transitivity, for any $x\in \beta$, we have  $x \in \alpha$ for every $\alpha \in S$, so that $\beta \subset \bigcap{S}$. This implies that $\bigcap{S}$ is transitive. Hence $\bigcap{S}$ is an ordinal. By our previous lemma, we see that $\bigcap{S}$ is minimal. 
\\ \\
It remains to show that $\ord$ is linearly ordered. By our previous lemma, it suffices to show that if $\alpha$ and $\beta$ are ordinals, then either $\alpha \subset \beta$ or $\beta \subset \alpha$. Suppose, towards a contradiction, that  both $\alpha \not \subset \beta$ and $\beta \not \subset \alpha$. Our last paragraph shows that $\alpha \cap \beta$ is an ordinal, which must be a proper subset both of $\alpha$ and of $\beta$. Our previous lemma implies that $\alpha \cap \beta \in \alpha$ and $\alpha \cap \beta \in \beta$. Hence $\alpha \cap \beta \in \alpha \cap \beta$, a contradiction. 
\end{proof}
...
\end{claim}
\begin{proof}
Now, suppose, towards a contradiction, that the set $D\coloneqq \{ x\in W \mid W_x$ is not isomorphic to an ordinal$\}$ is nonempty.   Let $m = \min(D)$. Then for each $y< m$, there is some ordinal $Z(y)$ such that $(W_m)_y = W_y \cong Z(y)$.  By our previous lemma, this implies that $W_m$ is isomorphic to some ordinal, contrary to our choice of $m$.
\end{proof}It follows from our previous lemma that $W \cong \alpha$ for some ordinal $\alpha$.
\end{proof}
...
\begin{corollary}[Burali-Forti]
$\ord$ is a proper class for otherwise $\sup{\ord}$ and $\sup{\ord}+1$ are ordinals by our last corollary, in which case $$\sup{\ord} < \sup{\ord} +1 \leq \sup{\ord},$$ a contradiction. 
\end{corollary}
...
\begin{corollary}(in $\zfc$)
Let $\alpha$ be an ordinal and let $\kappa$ and $\lambda$ be infinite cardinals.
...
\item Let $\si(\kappa)$ denote the statement that $\kappa$ is a weakly inaccessible cardinal. If $\si(\kappa)$, then $V_{\kappa} \models \zfc$.
...
\end{corollary}
\begin{proof}
Apply our last lemma with $\lambda =\omega$ and $\kappa = \omega$. Since $\omega \times \omega$ is countable, we're done.
\end{proof}
...
\end{proof}But $\cf(\kappa^+)$ is a cardinal  by our previous lemma. Hence $\cf(\kappa^+) \geq \kappa^+$.
\end{proof}
...
\end{corollary}
\begin{proof}
Using our last lemma, we have that $\bigcup{x} \in V$. Thus, $\bigcup{x}\subset V_{\beta}$ for some $\beta \in \ord$. This implies that $y \subset V_{\beta}$ for each $y\in x$. Hence $x\subset \P(V_{\beta}) = V_{\beta +1}$, so that $x \in \P(V_{\beta +1}) = V_{\beta +2}$.
\end{proof}
...
\begin{notation} $ $
\bi
\item For any language $\L$, we will write $\bar{x}$ for a finite sequence of symbols $(x_1, \ldots, x_k)$ in $\L$.  
\item For any set $A$, a symbol of the form $\bar{a}$ will refer to a finite sequence $(a_1, \ldots, a_k)$  in $A$. Abusing notation, we will  write $\bar{a} \in A$ to express that the components of $\bar{a}$ belong to $A$. Also, we will write $f(\bar{a})$ for the sequence $(f(a_1), \ldots, f(a_k))$.
\ei
\end{notation}
...
\begin{exmp}
Let $\kappa$ be a regular cardinal. Any element of the set $$\hc(\kappa) \coloneqq \{x \mid \lvert{\tc(x)}\rvert < \kappa\} $$ is said to have \textit{hereditary cardinality $<\kappa$}. Then $x \in \hc(\kappa)$ if and only if $x$ has cardinality $< \kappa$, every element of $x$ has cardinality $< \kappa$, every element of every element of $x$ has cardinality $< \kappa$, and so on. Note that $(\hc(\kappa), \in) $ is a transitive model for the language of set theory.  One can show that $\hc(\omega) = V_{\omega}$ and that $\hc(\kappa) =V_{\kappa}$ when $\kappa$ is inaccessible. 
\end{exmp}

\begin{theorem}
$\con(\zfc) \implies \con(\zfc - \mathsf{Reg})$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
Let $\mathsf{SI}$ denote the assertion that there exists a strongly inaccessible cardinal. If $\kappa$ is the least $\lambda$ such that $\si(\lambda)$, then both $V_{\kappa} \models \zfc$ and $V_{\kappa} \models \neg{\mathsf{SI}}$.
\end{theorem}
\begin{proof}
...
\begin{lemma}[Tarski-Vaught elementary submodel criterion]
...
\end{lemma}
\begin{proof}
Suppose that $E^A$ is both well-founded and extensional. We can extend transfinite induction (and thus recursion) on $\ord$ to any well-founded class.  Thus, we can define $f : A \to V$ by $$ f(a) = \{f(b) \mid b \in A \land b{E^A}a\}    .$$ Let $x\in f(A)$, so that $x = \{f(y) \mid y \in A \land y{E^A}z)\}$ for some $z\in A$. Hence any element of $x$ belongs to the image of $f$, which shows that $f(A)$ is a transitive class. 
\\ \\ Now, we want to show that $f$ is an embedding. Let $a, b\in A$ with $a\ne b$. Suppose, towards a contradiction, that the class $$D\coloneqq \{z \in f(A) \mid (\exists a,b \in A)(a\ne b \land z = f(a) = f(b))\}$$ is nonempty. Choose $m \in D$ of least rank and let $a$ and $b$ witness that $m \in D$. Since $a\ne b$, we have $\ext_{E^A}(a) \ne \ext_{E^A}(b)$. Assume, wlog, that there is some $u \in \ext_{\E^A}(a) \setminus \ext_{E^A}(b)$.  Then $$f(u) \in f(a) =m = f(b),$$ and thus there is some $v \in \ext_{E^A}(b)$ such that $s\coloneqq f(u) = f(v)$.  Since $u \ne v$, we have $s \in D$. But $s\in m$, so that $\rnk(s) < \rnk(m)$, a contradiction. It follows that $f$ is injective. 
...
\begin{term}
We call $(B, E^B)$ from our previous lemma the \textit{transitive collapse of $A$}.
\end{term}
...
\end{prop}
\begin{proof}
In general, as a result of the compactness theorem, if an $\L$-structure $(A, E^A)$ has a chain $$x_1{E^A}x_2{E^A}\cdots {E^A}x_n$$ for each $n\in \N$, then there is some $\L$-structure $(B, E^B)$ such that   $A\equiv B$ but $E^B$ is not well-founded.
\end{proof}
...
\begin{note}
Let $M$ be a transitive class. Any $\Delta_0$-formula is absolute for $M$.
\end{note}

\begin{exmp} The following expressions can be written as $\Delta_0$-formulas and thus are absolute for any transitive model. 
\be
...
\begin{note}
For every formula $\varphi(\bar{x})$ and every ordinal $\alpha$, there is some limit ordinal $\beta > \alpha$ such that for any $\bar{b} \in V_{\beta}$, we have $\varphi^V[\bar{b}] \iff \varphi[\bar{b}]$.
\end{note}
...
\begin{remark}
Our proof of the reflection principle can be adapted to any cumulative hierarchy.
\end{remark}
...
Discussion of Homework 2, Problems 1 and 4.

\subsection{Lecture 28}
Further discussion of Homework 2, Problem 4. Return to $L$.
...
(Homework 3 assigned before Lecture 34.)
...
Cohen proved that $$\con(\zf) \implies \con(\zfc + \neg{\mathsf{CH}}),$$ thereby proving that $\mathsf{CH}$ (as well as $\mathsf{GCH}$) is independent of $\zfc$.  In fact, assuming that $\zf$ is consistent, we have both that $\zfc + (2^{\aleph_0} = \aleph_{\alpha +1})$ is consistent for any $\alpha$ and that $\zfc + (2^{\aleph_0} = \aleph_{\lambda})$ is consistent for any $\lambda$ with $\cf(\lambda) > \omega$.
