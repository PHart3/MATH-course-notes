...
\begin{definition}
Let  $\left(P, <\right)$ be any linear ordering. A \textit{Dedekind cut in $P$} is a pair $(A, B)$ of disjoint nonempty subsets of $P$ such that
...
\begin{proof} $ $
\begin{enumerate}[label=(\arabic*)]
\item Let $f: \left(W, <\right) \to (W,<)$ be an automorphism.  Let $x\in W$. By \cref{woprop}, we see that $f(x)\geq x$ and $x = f^{-1}(f(x))\geq f(x)$. Hence $f(x) =x$.
...
\begin{definition}
Let $\left(P, <\right)$ be a partially ordered set. A \textit{chain in $P$} is a nonempty subset $X\subset P$ such that $(X, <\restriction_X)$ is a linear ordering. 
\end{definition}
...
\\ \\ It remains to show that $(x,y) \in E^A \iff f(x)\in f(y)$. The forward direction is obvious. For the backward direction, suppose that $f(x) \in f(y)$. Then $f(x) = f(c)$ for some $c{E^A}y$. Since $f$ is injective, we have $x=c$. Thus, $(x,y) \in E^A$. 
\end{proof}
...
\textit{Discussion of Homework 3 solutions.}

\begin{problem}[The hierarchy of constructible sets.]

Prove in $\zf + \VL$ that for any $\alpha < \aleph_1$, there is some $\alpha < \beta < \aleph_1$ such that for any $x\subset \omega$, if $x\in L_{\beta+1}$, then $x\in L_{\beta}$.
\end{problem}
\begin{solution}
Recall that $\zf+ \VL \models \ac$, so that \cref{LS} holds. We know that $\P(\omega)\subset L_{\omega_1}$. Note that no $\theta < \omega_1$ satisfies $\P(\omega)\subset L_{\theta}$ because $\lvert{L_{\theta}}\rvert<\aleph_1$.
