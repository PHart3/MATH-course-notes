...
Consider  a class model $(A, E^A)$ of $\zf$, where $E^A$ is a (strict) binary relation interpreting $\in$. If $E^A$ is both well-founded and extensional, then there is some transitive class model $\left(B, \in_B\right)$  such that $A \cong B$.
\end{lemma}
\begin{proof}
...
In 1939, G\"odel proved that $\zf \vdash \left(\zf\right)^L + \left(V=L\right)^L$, which means that if $\zf$ is consistent, then so is $\zf + {V=L}$. The assertion ``$V=L$" is known as the \textit{axiom of constructibility}.
