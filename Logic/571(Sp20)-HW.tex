...
\]
\begin{definition}
Suppose that $\L$ is a finite language without function symbols. Let $\D$ and $\E$ be two $\L$-structures. Let $n\in \omega$. The \textit{Ehrenfeucht-Fra\"{\i}ss\'e  game $G_n(\D, \E)$ of length $n$ on $\D$ and $\E$} is a game of perfect information played as follows.
...
Consider the Ehrenfeucht-Fra\"{\i}ss\'e  game $G_n(\E, \N^{\ast})$ of length $n\in \omega$ on $\E$ and $\N^{\ast}$. The duplicator has a winning strategy in $G_n(\E, \N^{\ast})$ by adhering to the following rules.
